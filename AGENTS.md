...
Branche de travail dédiée au chantier agent :
...
L’objectif est de travailler sur la totalité des sorts ADD2E sans régression du système Foundry.
...
- vérifier les images de sorts dans le bon répertoire ;
...
### 2.1. Manuel des joueurs AD&D 2e

Le Manuel des joueurs est la source de vérité prioritaire pour tout ce qui concerne les règles ADD2E.

Cela inclut notamment :

- les noms exacts des sorts ;
- le nombre de sorts par classe et par niveau ;
...
- les composantes verbales, somatiques et matérielles ;
- les composants consommés ou non consommés ;
- les jets de sauvegarde ;
- les effets mécaniques ;
- les restrictions ;
- les exceptions ;
- les versions inverses des sorts ;
- les descriptions réelles.

Si un export Foundry, un script, un nom historique, un tag ou une automatisation contredit le Manuel des joueurs sur une règle ADD2E, l’agent doit signaler l’écart et préparer une correction alignée sur le Manuel des joueurs.

### 2.2. Fichiers JSON Foundry existants

Les fichiers JSON commençant par `fvtt-` sont la référence stricte pour la structure technique des objets Foundry.

Ils ne sont pas prioritaires sur le Manuel des joueurs pour les règles ADD2E.
...
Champs à respecter quand ils existent :
...
Le Guide du Maître sert de source complémentaire pour :
...
Le Guide du Maître ne doit pas être utilisé pour contredire une règle claire du Manuel des joueurs concernant les sorts de joueur.

### 2.4. Code existant du système

Le code existant du système ADD2E est la source de vérité technique. Il ne remplace pas les règles AD&D, mais il détermine comment une donnée ou un `onUse` doit être branché dans Foundry.
...
1. conserver le champ tel quel ou vide ;
...
- un `onUse` qui dépasse ce que le système sait résoudre.
...
Les corrections doivent rester chirurgicales. Éviter les refontes globales, les renommages massifs, les suppressions de champs existants et les déplacements de fichiers non demandés.
...
Les sorts de Clerc niveau 1 déjà corrigés servent de modèle validé pour la structure JSON, les chemins `onUse`, les scripts dans `scripts/sorts/`, les retours `true`/`false`, les messages, les effets et la compatibilité V13/V14/V15.
...
### 9.1. Emplacement réel des images
...
Le champ `img` du sort doit pointer vers le chemin Foundry complet correspondant :

```text
systems/add2e/assets/icones/sorts/<nom-du-sort>.webp
```
...
### 9.4. Cas où l’image ne doit pas être modifiée
...
## 11. Effets actifs, états, messages et permissions
...
Si le joueur ne possède pas l’autorité nécessaire, utiliser le relais MJ ou le système socket existant. Ne pas faire de mise à jour directe provoquant une erreur de permission.
...
## 14. Composantes de sorts

Pour les composantes :

- le Manuel des joueurs est la source prioritaire ;
...
Si le Manuel indique des composants alternatifs, ne pas les traiter comme des composants tous obligatoires simultanément.
...
## 16. Vérifications obligatoires avant proposition de push
...
Chemin d’image assets/icones/sorts concerné
...
Toute affirmation de test doit distinguer :
...
## 19. Règles spécifiques au chantier global des sorts
...
présent dans Manuel des joueurs
écart nom
écart nombre
écart composant
écart description
...
image assets/icones/sorts existante
statut
```
...
1. lire le champ `img` ;
2. vérifier si l’image pointe vers `systems/add2e/assets/icones/sorts/` ;
3. vérifier si le fichier image existe dans `assets/icones/sorts/` ;
...
## 20. Principe de sécurité contre les régressions
...
## 21. Convention de décision

Quand une règle est incertaine, l’ordre de décision est :

1. Manuel des joueurs pour toutes les règles ADD2E ;
2. Guide du Maître pour les cas d’arbitrage et les compléments non couverts par le Manuel ;
3. schéma JSON Foundry existant ;
4. script `onUse` existant dans `scripts/sorts/` ;
5. image existante dans `assets/icones/sorts/` ;
6. code moteur ADD2E existant ;
7. décision explicite de l’utilisateur ;
8. note `a_verifier`.
...
Toujours utiliser le Manuel des joueurs comme source de vérité pour toute règle ADD2E.

Toujours lire `scripts/sorts/` avant de créer ou corriger un `onUse`.

Toujours vérifier `assets/icones/sorts/` avant de créer ou remplacer une image de sort.
