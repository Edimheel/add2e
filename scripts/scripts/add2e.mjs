...
// ============================================================
globalThis.ADD2E_SPELL_PREPARATION_VERSION = "2026-05-03-spell-preparation-pools-v11-native-hbs-groups-feature-use-fix";
...
function add2eGetSlotsForEntryLevel(actor, entry, spellLevel) {
  ...
  const tryArray = (arr) => Array.isArray(arr) ? Number(arr[idx] || 0) || 0 : null;
...
function add2eEnhanceCharacterSheetUi(sheet, html) {
  ...
  const spellTab = sheetRoot.querySelector('[data-tab="sorts"], [data-tab="sort"], .tab.sorts, .tab[data-tab="sorts"]');
  const firstSortTable = sheetRoot.querySelector("table.sort-table");
  const spellContainer = spellTab || firstSortTable?.closest?.(".a2e-tab-content") || firstSortTable?.parentElement;
  ...
  for (const table of sheetRoot.querySelectorAll("table.sort-table")) {
    ...
    for (const row of table.querySelectorAll("tbody tr")) {
      ...
      if (memBadge && entry) {
        ...
        memBadge.textContent = `${entry.label} ${memCount}`;
...
for (const itemSource of itemsAvecPouvoirs) {
  ...
  const add2eBuildCountersForLevel = (spellLevel) => {
    return add2eSpellEntriesForHbs
      ...
      .filter(counter => counter.max > 0 || counter.count > 0 || add2eSpellEntriesForHbs.length > 1);
  };
  ...
  data.add2eSpellSummaryRows = add2eSpellEntriesForHbs.map(entry => {
    ...
    for (let spellLevel = 1; spellLevel <= maxSpellLevel; spellLevel++) {
      const count = add2eCountPreparedForEntryLevel(this.actor, entry, spellLevel);
      const max = add2eGetSlotsForEntryLevel(this.actor, entry, spellLevel);
      if (max > 0 || count > 0 || add2eSpellEntriesForHbs.length > 1) {
  ...
  for (let spellLevel = 1; spellLevel <= add2eMaxSpellLevelForHbs; spellLevel++) {
    ...
    for (const counter of counters) {
      ...
      if (counter.max > 0 || counter.count > 0 || groupedSorts.length) {
...
html.find('.sort-delete').off().on('click', async function(ev) {
  ...
  add2eV25RerenderActorSheet(self.actor);
  return false;
});
...
html.find('.sort-memorize-plus, .sort-memorize-minus')
  ...
  .on('click', async ev => {
    ...
    add2eV25RerenderActorSheet(this.actor);
  });
...
function add2eV25NormalizeSpellKey(value) {
  const v = String(value ?? "")
    .trim()
    .toLowerCase()
    .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
...
function add2eV25SpellLabel(value) {
  const key = add2eV25NormalizeSpellKey(value);
  const labels = {
    clerc: "Clerc",
    druide: "Druide",
    magicien: "Magicien",
...
function add2eV25ToArray(value) {
  if (Array.isArray(value)) return value.filter(v => v !== undefined && v !== null && String(v).trim() !== "");
  if (typeof value === "string") return value.split(/[,;|]/).map(s => s.trim()).filter(Boolean);
  return [];
}

function add2eV25GetClassItem(actor) {
  const classItems = actor?.items?.filter?.(i => String(i.type || "").toLowerCase() === "classe") ?? [];
  if (!classItems.length) return null;

  const wanted = String(actor?.system?.classe || actor?.system?.details_classe?.label || actor?.system?.details_classe?.name || "")
    .trim()
    .toLowerCase();

  if (wanted) {
    const found = classItems.find(i => String(i.name || "").trim().toLowerCase() === wanted);
    if (found) return found;
  }
...
function add2eV25GetClassSystem(actor) {
  const classItem = add2eV25GetClassItem(actor);
  return classItem?.system ?? actor?.system?.details_classe ?? {};
}

function add2eV25GetProgressionRow(actor) {
  const level = Math.max(1, Number(actor?.system?.niveau ?? 1) || 1);
  const details = add2eV25GetClassSystem(actor);
  const progression = Array.isArray(details?.progression) ? details.progression : [];
  return progression.find(r => Number(r.niveau ?? r.level) === level) ?? progression[level - 1] ?? {};
}

function add2eV25GetSpellcastingEntries(actor) {
  const details = add2eV25GetClassSystem(actor);
  const casting = details?.spellcasting ?? actor?.system?.spellcasting ?? {};
  ...
  if (rawEntries) {
    return rawEntries.map((e, index) => {
      const rawKey = e.key ?? e.list ?? e.liste ?? e.name ?? e.type ?? e.label;
      const key = add2eV25NormalizeSpellKey(rawKey);
      return {
        index,
        rawKey: String(rawKey ?? key),
        key,
        label: add2eV25SpellLabel(key),
        startsAt: Number(e.startsAt ?? e.startLevel ?? e.niveauDepart ?? casting.startsAt ?? 1) || 1,
        maxSpellLevel: Number(e.maxSpellLevel ?? e.maxLevel ?? e.maxNiveauSort ?? casting.maxSpellLevel ?? 0) || 0,
        slotsField: e.slotsField || e.slotField || e.progressionField || null
      };
  ...
  return [...new Set(add2eV25ToArray(casting.lists).map(add2eV25NormalizeSpellKey).filter(Boolean))]
    .map((key, index) => ({
      index,
      rawKey: key,
      key,
...
function add2eV25ReadSlots(actor, entry, spellLevel) {
  const row = add2eV25GetProgressionRow(actor);
  const key = add2eV25NormalizeSpellKey(entry?.key);
  const idx = Math.max(0, Number(spellLevel) - 1);

  const readArray = (arr) => Array.isArray(arr) ? (Number(arr[idx] ?? 0) || 0) : null;

  // Nouveau modèle : progression[].spellSlotsByList.{Druide/Magicien/Clerc}
  // Lecture robuste : on compare les clés normalisées, donc Druide, druide,
  // Druidique, Magicien ou wizard retombent sur la même entrée interne.
  for (const containerName of ["spellSlotsByList", "spellsByList", "spellsPerLevelByList", "sortsParListe"]) {
    const container = row?.[containerName];
    if (!container || typeof container !== "object") continue;

    for (const [rawContainerKey, value] of Object.entries(container)) {
      if (add2eV25NormalizeSpellKey(rawContainerKey) !== key) continue;
      const v = readArray(value);
      if (v !== null) return v;
    }
  }
  ...
  for (const field of directNames) {
    const v = readArray(row?.[field]);
    if (v !== null) return v;
  }

  // Ancien modèle : seulement pour une classe à une seule liste de sorts.
  if (add2eV25GetSpellcastingEntries(actor).length <= 1) {
    return readArray(row?.spellsPerLevel) ?? readArray(row?.sortsParNiveau) ?? 0;
  }

  return 0;
}

function add2eV25GetSpellListsForSpell(item) {
  const sys = item?.system ?? {};
  const raw = add2eV25ToArray(sys.spellLists);
  if (raw.length) return [...new Set(raw.map(add2eV25NormalizeSpellKey).filter(Boolean))];
  const legacy = sys.classe || sys.class || sys.liste;
  const key = add2eV25NormalizeSpellKey(legacy);
  return key ? [key] : [];
}

function add2eV25GetMatchingEntry(actor, sort) {
  const entries = add2eV25GetSpellcastingEntries(actor);
  const lists = add2eV25GetSpellListsForSpell(sort);
  return entries.find(e => lists.includes(e.key)) ?? entries[0] ?? null;
}

function add2eV25GetMemorizedCount(sort, entry = null) {
  try {
    if (entry && typeof add2eGetMemorizedCountForEntry === "function") {
      return add2eGetMemorizedCountForEntry(sort, entry);
    }
    if (typeof add2eGetTotalMemorizedCount === "function") {
      return add2eGetTotalMemorizedCount(sort);
    }
    return Number(sort?.getFlag?.("add2e", "memorizedCount") ?? 0) || 0;
  } catch (e) {
    return 0;
  }
}

function add2eV25BuildSpellPoolSummary(actor) {
  const entries = add2eV25GetSpellcastingEntries(actor);
  ...
  return entries.map(entry => {
    ...
    for (let spellLevel = 1; spellLevel <= maxSpellLevel; spellLevel++) {
      const max = actorLevel >= Number(entry.startsAt || 1)
        ? add2eV25ReadSlots(actor, entry, spellLevel)
        : 0;

      const prepared = (actor.items?.filter?.(i => i.type === "sort" && Number(i.system?.niveau || 1) === spellLevel) ?? [])
        .filter(i => add2eV25GetMatchingEntry(actor, i)?.key === entry.key)
        .reduce((sum, sort) => sum + add2eV25GetMemorizedCount(sort, entry), 0);

      levels.push({ spellLevel, prepared, max });
    }
  ...
  }).filter(group => group.maxSpellLevel > 0);
}

globalThis.add2eGetSpellPreparationSummary = add2eV25BuildSpellPoolSummary;

function add2eV25EscapeHtml(value) {
  return String(value ?? "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function add2eV25SpellPanelHtml(actor) {
  const groups = add2eV25BuildSpellPoolSummary(actor);
  if (!groups.length) return "";

  const lines = groups.map(group => {
    const levels = group.levels.map(l => {
      const over = l.max > 0 && l.prepared > l.max;
      const full = l.max > 0 && l.prepared >= l.max;
      const cls = over ? " over" : full ? " full" : "";
      return `<span class="a2e-spell-pool-v25-chip${cls}">N${l.spellLevel} ${l.prepared}/${l.max}</span>`;
    }).join(" ");

    return `<div class="a2e-spell-pool-v25-line"><strong>${add2eV25EscapeHtml(group.label)} :</strong> ${levels}</div>`;
  }).join("");

  return `<section class="a2e-spell-pools-v25" data-add2e-spell-pools-panel="1">${lines}</section>`;
}

function add2eV25InstallSpellPanelCss() {
  if (document.getElementById("add2e-spell-pools-v25-style")) return;
  const style = document.createElement("style");
  style.id = "add2e-spell-pools-v25-style";
  style.textContent = `
    ...
    .a2e-spell-pool-v25-line strong {
      min-width: 74px;
      color: #6f4b12;
      font-weight: 900;
    }
    .a2e-spell-pool-v25-chip {
      display: inline-flex;
      align-items: center;
      padding: 2px 7px;
    ...
    .a2e-spell-pool-v25-chip.full {
      background: #f3dfa5;
    }
    .a2e-spell-pool-v25-chip.over {
      background: #ffe0da;
      border-color: #c45b48;
      color: #8a1f18;
    }
...
function add2eV25NormalizeTabName(value) {
  return String(value ?? "")
    .trim()
    .toLowerCase()
    .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
...
function add2eV25FindTab(root, names) {
  const wanted = new Set(names.map(add2eV25NormalizeTabName));
  const candidates = [...root.querySelectorAll(".a2e-tab-content[data-tab], .tab[data-tab], section[data-tab], div[data-tab]")]
    .filter(el => !el.classList.contains("item") && !el.matches("a"));

  for (const el of candidates) {
    if (wanted.has(add2eV25NormalizeTabName(el.dataset.tab))) return el;
  }

  return null;
}

function add2eV25GetSheetRoot(html) {
...
function add2eV25FindPreparationsPanel(container) {
  if (!container) return null;

  const isPrepTitle = value => {
    const title = add2eV25NormalizeTabName(value);
    return title === "preparations" || title === "preparation" || title.includes("preparation");
  };

  // V28 : recherche stricte par titre direct, mais sans dépendre d'une classe CSS précise.
  // Cela évite de remplacer tout le résumé, tout en retrouvant les cartes "Préparations"
  // dont la classe varie selon les versions de character-sheet.hbs.
  const candidates = [];

  for (const panel of Array.from(container.querySelectorAll("section, article, div, aside"))) {
    if (!panel || panel.matches?.('[data-add2e-spell-pools-panel="1"]')) continue;
    if (panel.classList.contains("tab") || panel.classList.contains("a2e-tab-content") || panel.classList.contains("sheet-body")) continue;
    if (panel.querySelector?.(".sheet-tabs, .a2e-tabs")) continue;

    const titleEl = panel.querySelector?.(
      ":scope > h1, :scope > h2, :scope > h3, :scope > h4, :scope > header, :scope > .a2e-panel-title, :scope > .a2e-card-title, :scope > .a2e-panel-header, :scope > .a2e-card-header, :scope > .a2e-summary-title"
    );
    if (!titleEl || !isPrepTitle(titleEl.textContent)) continue;

    const nestedBigPanels = panel.querySelectorAll?.(":scope .a2e-panel, :scope .a2e-card, :scope section, :scope article")?.length ?? 0;
    const textLen = String(panel.textContent || "").trim().length;

    // Le bloc Préparations est une carte courte. Le résumé entier est beaucoup plus grand.
    if (textLen > 1800) continue;
    if (nestedBigPanels > 4) continue;

    candidates.push({ panel, textLen });
  }
...
function add2eV25ReplacePreparationsContent(actor, resumeTab) {
  const panelHtml = add2eV25SpellPanelHtml(actor);
  if (!panelHtml) return false;

  const prepPanel = add2eV25FindPreparationsPanel(resumeTab);
  if (!prepPanel) {
    console.warn("[ADD2E][SPELL PREP] Bloc Préparations introuvable dans le résumé : résumé laissé intact.");
    return false;
  }

  prepPanel.dataset.add2eSpellPrepHost = "1";

  const body = prepPanel.querySelector?.(":scope > .a2e-panel-body, :scope > .panel-body, :scope > .a2e-card-body, :scope > .content, :scope > .a2e-summary-card-body");
  if (body) {
    body.innerHTML = panelHtml;
    return true;
  }

  const titleEl = prepPanel.querySelector?.(
    ":scope > h2, :scope > h3, :scope > header, :scope > .a2e-panel-title, :scope > .a2e-card-title, :scope > .a2e-panel-header, :scope > .a2e-card-header"
  );

  // On conserve uniquement le titre du bloc Préparations, puis on remplace son contenu.
  for (const child of Array.from(prepPanel.children)) {
    if (child !== titleEl) child.remove();
  }

  prepPanel.insertAdjacentHTML("beforeend", panelHtml);
  return true;
}

function add2eV25InjectSpellPanels(app, html) {
  const actor = app?.actor ?? app?.document;
  if (!actor || actor.documentName !== "Actor" || actor.type !== "personnage") return;

  const root = add2eV25GetSheetRoot(html);
  if (!root) return;

  add2eV25InstallSpellPanelCss();

  // Reconstruction systématique : évite les panneaux périmés après changement de classe.
  for (const old of root.querySelectorAll('[data-add2e-spell-pools-panel="1"]')) old.remove();

  // Ancien bloc "Sorts utilisables" : remplacé par les lignes compactes Druide/Magicien.
  for (const old of root.querySelectorAll(".a2e-spellcasting-summary")) old.remove();

  const panelHtml = add2eV25SpellPanelHtml(actor);
  if (!panelHtml) return;

  const sortsTab = add2eV25FindTab(root, ["sorts", "sort"]);
  if (sortsTab) sortsTab.insertAdjacentHTML("afterbegin", panelHtml);

  const resumeTab = add2eV25FindTab(root, ["resume", "résumé", "summary", "sommaire"]);
  if (resumeTab) {
    // V27 : ne jamais insérer le panneau en haut du résumé en fallback.
    // S'il ne trouve pas la carte Préparations, il ne touche pas au résumé.
    add2eV25ReplacePreparationsContent(actor, resumeTab);
  }
}

function add2eV25InjectIntoRenderedSheet(actor) {
  if (!actor || actor.documentName !== "Actor" || actor.type !== "personnage") return;
  ...
  try {
    add2eEnhanceCharacterSheetUi(sheet, element);
  } catch (err) {
    console.warn("[ADD2E][SPELL PREP] Ré-application UI sorts impossible :", err);
  }

  try {
    add2eV25InjectSpellPanels(sheet, element);
  } catch (err) {
    console.warn("[ADD2E][SPELL PREP] Ré-injection panneaux impossible :", err);
  }
}

function add2eV25RerenderActorSheet(actor) {
  if (!actor || actor.documentName !== "Actor" || actor.type !== "personnage") return;
  if (!actor.sheet?.rendered) return;

  try {
    actor.sheet._add2eRememberActiveTab?.(actor.sheet.element);
  } catch (e) {}

  setTimeout(() => actor.sheet?.render?.(true), 40);
  setTimeout(() => add2eV25InjectIntoRenderedSheet(actor), 180);
  setTimeout(() => add2eV25InjectIntoRenderedSheet(actor), 450);
  setTimeout(() => add2eV25InjectIntoRenderedSheet(actor), 900);
}

Hooks.on("renderActorSheet", (app, html) => {
  add2eV25InjectSpellPanels(app, html);
  setTimeout(() => add2eV25InjectSpellPanels(app, html), 120);
  setTimeout(() => add2eV25InjectIntoRenderedSheet(app?.actor ?? app?.document), 350);
  setTimeout(() => add2eV25InjectIntoRenderedSheet(app?.actor ?? app?.document), 800);
});
...
Hooks.on("updateActor", (actor, changes, options) => {
  if (options?.add2eNoSpellPanelRefresh) return;
  if (foundry.utils.hasProperty(changes, "system.niveau") || foundry.utils.hasProperty(changes, "system.details_classe") || foundry.utils.hasProperty(changes, "system.classe")) {
    add2eV25RerenderActorSheet(actor);
  }
});

Hooks.on("createItem", (item) => {
  if (item?.parent?.documentName === "Actor" && ["classe", "sort"].includes(item.type)) add2eV25RerenderActorSheet(item.parent);
});
...
console.log("ADD2E | Spell preparation panels V29 refresh after spell change loaded");

// ============================================================
// ADD2E — V30 : Onglet Sorts groupé par niveau puis par liste
// Affichage demandé :
// Sorts de niveau 1 : Druide 0/2 Magicien 0/2
//   lignes de sorts du niveau 1
// Sorts de niveau 2 : ...
// ============================================================

globalThis.ADD2E_SPELL_PREPARATION_VERSION = "2026-05-03-spell-preparation-pools-v11-native-hbs-groups-feature-use-fix";

function add2eV30Escape(value) {
  if (typeof add2eV25EscapeHtml === "function") return add2eV25EscapeHtml(value);
  return String(value ?? "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function add2eV30Normalize(value) {
  if (typeof add2eV25NormalizeSpellKey === "function") return add2eV25NormalizeSpellKey(value);
  return String(value ?? "")
    .trim()
    .toLowerCase()
    .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
...
function add2eV30GetSheetRoot(html) {
  if (typeof add2eV25GetSheetRoot === "function") return add2eV25GetSheetRoot(html);
  const root = html?.[0] ?? html;
  if (!root || !(root instanceof HTMLElement)) return null;
  if (root.matches?.(".add2e-character-v3")) return root;
  return root.querySelector?.(".add2e-character-v3") ?? null;
...
function add2eV30FindTab(root, names) {
  if (typeof add2eV25FindTab === "function") return add2eV25FindTab(root, names);
  const wanted = new Set(names.map(v => add2eV30Normalize(v)));
  return [...root.querySelectorAll(".a2e-tab-content[data-tab], .tab[data-tab], section[data-tab], div[data-tab]")]
    .find(el => wanted.has(add2eV30Normalize(el.dataset.tab))) ?? null;
}

function add2eV30GetSpellEntries(actor) {
  if (typeof add2eV25GetSpellcastingEntries === "function") return add2eV25GetSpellcastingEntries(actor);
  if (typeof add2eGetSpellcastingEntries === "function") return add2eGetSpellcastingEntries(actor);
  return [];
}

function add2eV30GetSpellLists(sort) {
  if (typeof add2eV25GetSpellListsForSpell === "function") return add2eV25GetSpellListsForSpell(sort);
  if (typeof add2eGetSpellListsFromItem === "function") return add2eGetSpellListsFromItem(sort);
  const raw = sort?.system?.spellLists;
  const arr = Array.isArray(raw) ? raw : typeof raw === "string" ? raw.split(/[,;|]/) : [];
  return arr.map(add2eV30Normalize).filter(Boolean);
}

function add2eV30ReadSlots(actor, entry, spellLevel) {
  if (typeof add2eGetSlotsForEntryLevel === "function") return add2eGetSlotsForEntryLevel(actor, entry, spellLevel);
  if (typeof add2eV25ReadSlots === "function") return add2eV25ReadSlots(actor, entry, spellLevel);
  return 0;
}

function add2eV30CountPrepared(actor, entry, spellLevel) {
  if (typeof add2eCountPreparedForEntryLevel === "function") return add2eCountPreparedForEntryLevel(actor, entry, spellLevel);
  const key = add2eV30Normalize(entry?.key);
  return (actor.items?.filter?.(i => i.type === "sort" && Number(i.system?.niveau ?? i.system?.level ?? 1) === Number(spellLevel)) ?? [])
    .filter(sort => add2eV30GetSpellLists(sort).includes(key))
    .reduce((sum, sort) => sum + add2eV30GetMemorized(sort, entry), 0);
}

function add2eV30GetMemorized(sort, entry) {
  if (typeof add2eGetMemorizedCountForEntry === "function") return add2eGetMemorizedCountForEntry(sort, entry);
  if (typeof add2eV25GetMemorizedCount === "function") return add2eV25GetMemorizedCount(sort, entry);
  return Number(sort?.getFlag?.("add2e", "memorizedCount") ?? sort?.flags?.add2e?.memorizedCount ?? 0) || 0;
}

function add2eV30GetAllowedEntriesForSpell(actor, sort) {
  const actorLevel = Math.max(1, Number(actor?.system?.niveau ?? 1) || 1);
  const spellLevel = Number(sort?.system?.niveau ?? sort?.system?.level ?? 1) || 1;
  const spellLists = add2eV30GetSpellLists(sort);
  const entries = add2eV30GetSpellEntries(actor);

  return entries.filter(entry => {
    const key = add2eV30Normalize(entry.key);
    const startsAt = Number(entry.startsAt ?? 1) || 1;
    const maxSpellLevel = Number(entry.maxSpellLevel ?? 0) || 0;
    return spellLists.includes(key) && actorLevel >= startsAt && (!maxSpellLevel || spellLevel <= maxSpellLevel);
  });
}

function add2eV30BuildLevelCounters(actor, spellLevel) {
  const entries = add2eV30GetSpellEntries(actor)
    .filter(entry => {
      const maxSpellLevel = Number(entry.maxSpellLevel ?? 0) || 0;
      return !maxSpellLevel || Number(spellLevel) <= maxSpellLevel;
  ...
  return entries.map(entry => {
    const prepared = add2eV30CountPrepared(actor, entry, spellLevel);
    const max = add2eV30ReadSlots(actor, entry, spellLevel);
    return {
      key: add2eV30Normalize(entry.key),
      label: entry.label || (typeof add2eV25SpellLabel === "function" ? add2eV25SpellLabel(entry.key) : entry.key),
      prepared,
      max
    };
  }).filter(c => c.max > 0 || c.prepared > 0 || add2eV30GetSpellEntries(actor).length > 1);
}

function add2eV30BuildSpellRow(actor, sort) {
  const spellLevel = Number(sort?.system?.niveau ?? sort?.system?.level ?? 1) || 1;
  const allowedEntries = add2eV30GetAllowedEntriesForSpell(actor, sort);
  const lists = add2eV30GetSpellLists(sort);
  const labels = allowedEntries.length
    ? allowedEntries.map(e => e.label || e.key).join(" / ")
    : lists.map(k => typeof add2eV25SpellLabel === "function" ? add2eV25SpellLabel(k) : k).join(" / ") || "Non autorisé";

  const prepControls = allowedEntries.length ? allowedEntries.map(entry => {
    const entryKey = add2eV30Normalize(entry.key);
    const cur = add2eV30GetMemorized(sort, entry);
    const max = add2eV30ReadSlots(actor, entry, spellLevel);
    const preparedAtLevel = add2eV30CountPrepared(actor, entry, spellLevel);
    const over = max > 0 && preparedAtLevel > max;
    return `
      <span class="a2e-v30-prep-unit${over ? " over" : ""}" title="${add2eV30Escape(entry.label)} : ${preparedAtLevel}/${max} emplacements utilisés au niveau ${spellLevel}">
        <button type="button" class="a2e-v30-prep-btn a2e-v30-prep-minus" data-sort-id="${add2eV30Escape(sort.id)}" data-entry-key="${add2eV30Escape(entryKey)}">−</button>
        <span class="a2e-v30-prep-badge">${add2eV30Escape(entry.label)} ${cur}</span>
        <button type="button" class="a2e-v30-prep-btn a2e-v30-prep-plus" data-sort-id="${add2eV30Escape(sort.id)}" data-entry-key="${add2eV30Escape(entryKey)}">+</button>
      </span>`;
  }).join("") : `<span class="a2e-v30-spell-locked">${add2eV30Escape(labels)}</span>`;

  const school = sort.system?.école || sort.system?.ecole || sort.system?.school || "";

  return `
    <article class="a2e-v30-spell-row" data-sort-id="${add2eV30Escape(sort.id)}">
      <div class="a2e-v30-spell-main">
        <img src="${add2eV30Escape(sort.img || "icons/svg/book.svg")}" alt="" class="a2e-v30-spell-img">
        <div class="a2e-v30-spell-text">
          <a href="#" class="a2e-v30-spell-name" data-sort-id="${add2eV30Escape(sort.id)}">${add2eV30Escape(sort.name)}</a>
          <span class="a2e-v30-spell-meta">${add2eV30Escape(labels)}${school ? " — " + add2eV30Escape(school) : ""}</span>
        </div>
      </div>
      <div class="a2e-v30-spell-prep">${prepControls}</div>
      <div class="a2e-v30-spell-actions">
        <button type="button" class="a2e-v30-cast" data-sort-id="${add2eV30Escape(sort.id)}" title="Lancer le sort"><i class="fas fa-magic"></i></button>
        <button type="button" class="a2e-v30-edit" data-sort-id="${add2eV30Escape(sort.id)}" title="Éditer"><i class="fas fa-edit"></i></button>
        <button type="button" class="a2e-v30-delete" data-sort-id="${add2eV30Escape(sort.id)}" title="Supprimer"><i class="fas fa-trash"></i></button>
      </div>
    </article>`;
}

function add2eV30BuildGroupedSpellsPanel(actor) {
  const entries = add2eV30GetSpellEntries(actor);
  if (!entries.length) return "";

  const spells = (actor.items?.filter?.(i => i.type === "sort") ?? [])
    .sort((a, b) => (Number(a.system?.niveau ?? a.system?.level ?? 1) || 1) - (Number(b.system?.niveau ?? b.system?.level ?? 1) || 1) || String(a.name).localeCompare(String(b.name)));

  const maxEntryLevel = Math.max(0, ...entries.map(e => Number(e.maxSpellLevel ?? 0) || 0));
  const maxSpellItemLevel = Math.max(0, ...spells.map(s => Number(s.system?.niveau ?? s.system?.level ?? 1) || 1));
  const maxLevel = Math.max(maxEntryLevel, maxSpellItemLevel);
  if (!maxLevel) return "";

  const sections = [];
  for (let spellLevel = 1; spellLevel <= maxLevel; spellLevel++) {
    const counters = add2eV30BuildLevelCounters(actor, spellLevel);
    if (!counters.length && !spells.some(s => Number(s.system?.niveau ?? s.system?.level ?? 1) === spellLevel)) continue;

    const counterHtml = counters.map(c => {
      const over = c.max > 0 && c.prepared > c.max;
      const full = c.max > 0 && c.prepared >= c.max;
      return `<span class="a2e-v30-level-chip${over ? " over" : full ? " full" : ""}">${add2eV30Escape(c.label)} ${Number(c.prepared) || 0}/${Number(c.max) || 0}</span>`;
    }).join(" ");

    const levelSpells = spells.filter(s => Number(s.system?.niveau ?? s.system?.level ?? 1) === spellLevel);
    const spellRows = levelSpells.length
      ? levelSpells.map(sort => add2eV30BuildSpellRow(actor, sort)).join("")
      : `<div class="a2e-v30-no-spells">Aucun sort de niveau ${spellLevel} sur l’acteur.</div>`;

    sections.push(`
      <section class="a2e-v30-spell-level" data-spell-level="${spellLevel}">
        <header class="a2e-v30-level-header">
          <strong>Sorts de niveau ${spellLevel} :</strong>
          <span class="a2e-v30-level-counters">${counterHtml}</span>
        </header>
        <div class="a2e-v30-spell-slots">${spellRows}</div>
      </section>`);
  }

  return `<section class="a2e-v30-spells-by-level" data-add2e-v30-spells-by-level="1">${sections.join("")}</section>`;
}

function add2eV30InstallCss() {
  if (document.getElementById("add2e-spell-v30-style")) return;
  const style = document.createElement("style");
  style.id = "add2e-spell-v30-style";
  style.textContent = `
    .add2e-character-v3 [data-add2e-v30-hidden-old-spells="1"] { display: none !important; }
    .add2e-character-v3 .a2e-v30-spells-by-level {
      margin: 8px 0 10px 0;
      border: 1px solid #d9bf73;
      border-radius: 10px;
      background: rgba(255,250,235,0.97);
      overflow: hidden;
      box-shadow: 0 1px 5px rgba(80,58,10,0.10);
    }
    .add2e-character-v3 .a2e-v30-spell-level + .a2e-v30-spell-level { border-top: 1px solid #d9bf73; }
    .add2e-character-v3 .a2e-v30-level-header {
      display: flex;
      align-items: center;
      flex-wrap: wrap;
      gap: 7px;
      padding: 7px 9px;
      background: rgba(240,224,169,0.68);
      color: #6f4b12;
      font-weight: 900;
    }
    .add2e-character-v3 .a2e-v30-level-counters { display: inline-flex; flex-wrap: wrap; gap: 5px; }
    .add2e-character-v3 .a2e-v30-level-chip,
    .add2e-character-v3 .a2e-v30-prep-badge {
      display: inline-flex;
      align-items: center;
      padding: 2px 7px;
      border: 1px solid #c49a35;
    ...
    .add2e-character-v3 .a2e-v30-level-chip.full { background: #f3dfa5; }
    .add2e-character-v3 .a2e-v30-level-chip.over,
    .add2e-character-v3 .a2e-v30-prep-unit.over .a2e-v30-prep-badge {
      background: #ffe0da;
      border-color: #c45b48;
      color: #8a1f18;
    }
    .add2e-character-v3 .a2e-v30-spell-slots { padding: 6px 8px; }
    .add2e-character-v3 .a2e-v30-spell-row {
      display: grid;
      grid-template-columns: minmax(240px, 1fr) auto auto;
      align-items: center;
      gap: 8px;
      padding: 5px 6px;
      border: 1px solid rgba(217,191,115,0.85);
      border-radius: 8px;
      background: #fffdf6;
      margin: 4px 0;
    }
    .add2e-character-v3 .a2e-v30-spell-main { display: flex; align-items: center; gap: 7px; min-width: 0; }
    .add2e-character-v3 .a2e-v30-spell-img { width: 28px; height: 28px; border: 0; object-fit: contain; flex: 0 0 auto; }
    .add2e-character-v3 .a2e-v30-spell-text { min-width: 0; display: flex; flex-direction: column; line-height: 1.2; }
    .add2e-character-v3 .a2e-v30-spell-name { font-weight: 900; color: #3d2b0a; text-decoration: none; }
    .add2e-character-v3 .a2e-v30-spell-name:hover { text-decoration: underline; }
    .add2e-character-v3 .a2e-v30-spell-meta { color: #7f704d; font-size: 0.82em; }
    .add2e-character-v3 .a2e-v30-spell-prep { display: inline-flex; flex-wrap: wrap; gap: 5px; justify-content: flex-end; }
    .add2e-character-v3 .a2e-v30-prep-unit { display: inline-flex; align-items: center; gap: 3px; }
    .add2e-character-v3 .a2e-v30-prep-btn,
    .add2e-character-v3 .a2e-v30-cast,
    .add2e-character-v3 .a2e-v30-edit,
    .add2e-character-v3 .a2e-v30-delete {
      width: 24px;
      height: 24px;
      padding: 0;
      display: inline-flex;
    ...
    .add2e-character-v3 .a2e-v30-cast { background: #e7f0ff; border-color: #7b9ed0; color: #184a82; }
    .add2e-character-v3 .a2e-v30-delete { background: #ffe9e6; border-color: #c27a70; color: #8a1f18; }
    .add2e-character-v3 .a2e-v30-spell-actions { display: inline-flex; gap: 4px; justify-content: flex-end; }
    .add2e-character-v3 .a2e-v30-no-spells {
      padding: 8px 10px;
      color: #7f704d;
      font-style: italic;
    }
...
function add2eV30HideLegacySortsPanel(sortsTab) {
  if (!sortsTab) return;
  const panels = Array.from(sortsTab.querySelectorAll(":scope > section, :scope > article, :scope > div"));
  for (const panel of panels) {
    if (panel.matches?.('[data-add2e-spell-pools-panel="1"], [data-add2e-v30-spells-by-level="1"]')) continue;
    if (panel.classList.contains("a2e-v30-spells-by-level")) continue;
    const titleEl = panel.querySelector?.(":scope > h1, :scope > h2, :scope > h3, :scope > header, :scope > .a2e-panel-title, :scope > .a2e-panel-header, :scope > .a2e-card-title, :scope > .a2e-card-header");
    const title = add2eV30Normalize(titleEl?.textContent ?? "");
    if (title === "sorts" || title === "sort") {
      panel.dataset.add2eV30HiddenOldSpells = "1";
    }
  }
}

function add2eV30BindSpellPanel(actor, root) {
  if (!actor || !root) return;

  root.querySelectorAll(".a2e-v30-prep-plus, .a2e-v30-prep-minus").forEach(btn => {
    btn.onclick = async ev => {
      ev.preventDefault();
      ev.stopPropagation();
      const sortId = btn.dataset.sortId;
      const entryKey = add2eV30Normalize(btn.dataset.entryKey);
      const sort = actor.items.get(sortId);
      if (!sort) return ui.notifications.warn("Sort introuvable.");

      const entry = add2eV30GetSpellEntries(actor).find(e => add2eV30Normalize(e.key) === entryKey);
      if (!entry) return ui.notifications.warn("Liste de préparation introuvable.");

      const spellLevel = Number(sort.system?.niveau ?? sort.system?.level ?? 1) || 1;
      let cur = add2eV30GetMemorized(sort, entry);

      if (btn.classList.contains("a2e-v30-prep-plus")) {
        const limit = add2eV30ReadSlots(actor, entry, spellLevel);
        const total = add2eV30CountPrepared(actor, entry, spellLevel);
      ...
      if (typeof add2eSetMemorizedCountForEntry === "function") {
        await add2eSetMemorizedCountForEntry(sort, entry, cur);
      } else {
        await sort.setFlag("add2e", "memorizedCount", cur);
      }

      if (typeof add2eV25RerenderActorSheet === "function") add2eV25RerenderActorSheet(actor);
      else actor.sheet?.render?.(true);
    };
  });

  root.querySelectorAll(".a2e-v30-spell-name, .a2e-v30-edit").forEach(btn => {
    btn.onclick = ev => {
      ev.preventDefault();
      ev.stopPropagation();
      const sort = actor.items.get(btn.dataset.sortId);
      if (sort) sort.sheet.render(true);
    };
  });

  root.querySelectorAll(".a2e-v30-delete").forEach(btn => {
    btn.onclick = async ev => {
      ev.preventDefault();
      ev.stopPropagation();
      const sortId = btn.dataset.sortId;
      if (!sortId) return;
      await actor.deleteEmbeddedDocuments("Item", [sortId]);
      if (typeof add2eV25RerenderActorSheet === "function") add2eV25RerenderActorSheet(actor);
      else actor.sheet?.render?.(true);
    };
  });

  root.querySelectorAll(".a2e-v30-cast").forEach(btn => {
    btn.onclick = async ev => {
      ev.preventDefault();
      ev.stopPropagation();
      const sort = actor.items.get(btn.dataset.sortId);
      if (!sort) return ui.notifications.warn("Sort introuvable.");
      if (typeof globalThis.add2eCastSpell !== "function") return ui.notifications.error("La fonction add2eCastSpell est introuvable.");
...
function add2eV30InjectGroupedSpells(app, html) {
  const actor = app?.actor ?? app?.document;
  if (!actor || actor.documentName !== "Actor" || actor.type !== "personnage") return;

  const root = add2eV30GetSheetRoot(html);
  if (!root) return;

  const sortsTab = add2eV30FindTab(root, ["sorts", "sort"]);
  if (!sortsTab) return;

  add2eV30InstallCss();

  for (const old of sortsTab.querySelectorAll('[data-add2e-v30-spells-by-level="1"]')) old.remove();
  add2eV30HideLegacySortsPanel(sortsTab);

  const htmlPanel = add2eV30BuildGroupedSpellsPanel(actor);
  if (!htmlPanel) return;

  const summary = sortsTab.querySelector('[data-add2e-spell-pools-panel="1"]');
  if (summary) summary.insertAdjacentHTML("afterend", htmlPanel);
  else sortsTab.insertAdjacentHTML("afterbegin", htmlPanel);

  add2eV30BindSpellPanel(actor, sortsTab);
}

Hooks.on("renderActorSheet", (app, html) => {
  setTimeout(() => add2eV30InjectGroupedSpells(app, html), 30);
  setTimeout(() => add2eV30InjectGroupedSpells(app, html), 220);
  setTimeout(() => add2eV30InjectGroupedSpells(app, html), 700);
});
...
Hooks.on("createItem", item => {
  if (item?.parent?.documentName === "Actor" && item.type === "sort") setTimeout(() => add2eV30InjectGroupedSpells(item.parent.sheet, item.parent.sheet?.element?.[0] ?? item.parent.sheet?.element), 250);
});
Hooks.on("updateItem", item => {
  if (item?.parent?.documentName === "Actor" && item.type === "sort") setTimeout(() => add2eV30InjectGroupedSpells(item.parent.sheet, item.parent.sheet?.element?.[0] ?? item.parent.sheet?.element), 250);
});
...
console.log("ADD2E | Spell preparation panels V30 level grouped spells loaded");


// ============================================================
// ADD2E V31 — HBS natif pour les sorts : désactive l'injection
// visuelle post-render et conserve seulement les handlers.
// ============================================================
globalThis.ADD2E_SPELL_PREPARATION_VERSION = "2026-05-03-spell-preparation-pools-v11-native-hbs-groups-feature-use-fix";

try {
  if (typeof add2eV25InjectSpellPanels === "function") add2eV25InjectSpellPanels = function() {};
  if (typeof add2eV25InjectIntoRenderedSheet === "function") add2eV25InjectIntoRenderedSheet = function() {};
  if (typeof add2eV30InjectGroupedSpells === "function") add2eV30InjectGroupedSpells = function() {};
} catch (err) {
  console.warn("[ADD2E][SPELLS][HBS NATIVE] Impossible de neutraliser les anciennes injections", err);
}
