/** ADD2E — Perception des alignements — Clerc niveau 2 — DialogV2 + VFX — V13/V14/V15 */
const __add2eOnUseResult = await (async () => {
  const SPELL = {"id":"perception-alignements","name":"Perception des alignements","fx":"detection","color":"#d7a7ff","min":1,"max":10};
  const RULES = ["Durée : 1 tour ; une créature examinée par round, jusqu’à 10.","Le MJ révèle l’alignement exact ; aucune donnée d’acteur n’est lue automatiquement.","La forme inverse peut cacher l’alignement pendant la durée répartie.","Certains objets magiques peuvent annuler le pouvoir du sort."];
  ...
  const details = [];
  details.push(`Créatures à examiner : <b>${targets.length} / 10</b>.`);
...
if (__add2eOnUseResult !== true && __add2eOnUseResult !== false) { console.error("[ADD2E][ONUSE][BAD_RETURN_STRICT]", { script: "connaissance-des-alignements.js", result: __add2eOnUseResult }); return false; }
