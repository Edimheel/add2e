// escalade_araignee.js - applique l'effet d'escalade sur la cible (ou le lanceur)
...
if (typeof item !== "undefined" && item?.name) {
  ...
  ui.notifications.warn("Impossible de trouver l'item du sort Escalade d’araignée.");
  return false;
}

let sourceActor = actor ?? _item?.parent;
if (!sourceActor) {
  ui.notifications.warn("Impossible de trouver l'acteur lanceur pour Escalade d’araignée.");
  return false;
}
...
// Nettoyage
const labelMatch = "escalade d’araignée";
const existing = targetActor.effects.filter(e =>
  (e.label || e.name || "").toLowerCase().includes("escalade")
);
if (existing.length) {
...
if (!effects.length) {
  ui.notifications.warn("Aucun effet à appliquer depuis le sort Escalade d’araignée.");
  return false;
}
...
ui.notifications.info(`${targetActor.name} peut grimper aux murs comme une araignée.`);
return true;
