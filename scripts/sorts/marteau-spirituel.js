/** ADD2E — Marteau spirituel — Clerc niveau 2 — DialogV2 + VFX — V13/V14/V15 */
const __add2eOnUseResult = await (async () => {
  const SPELL = {"id":"marteau-spirituel","name":"Marteau spirituel","fx":"projectile_magique","color":"#e8e8ff","min":1,"max":1};
  const RULES = ["Durée : 1 round par niveau, tant que le clerc se concentre.","Dégâts : 1d6 contre P-M ; 1d4 contre G.","Le marteau frappe exactement comme le clerc, sans bonus propre au toucher ni aux dégâts.","Pour franchir une immunité aux armes normales seulement : +1 magique par tranche de 3 niveaux.","Le marteau de guerre normal est lancé puis disparaît."];
  ...
  const details = [];
  details.push(`Durée : <b>${level} round(s)</b> ; valeur magique contre immunités : <b>+${Math.floor(level / 3)}</b>.`);
...
if (__add2eOnUseResult !== true && __add2eOnUseResult !== false) { console.error("[ADD2E][ONUSE][BAD_RETURN_STRICT]", { script: "marteau-spirituel.js", result: __add2eOnUseResult }); return false; }
