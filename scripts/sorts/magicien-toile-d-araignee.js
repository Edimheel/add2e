...
// Version : 2026-05-28-magicien-attaque-n2-toile-araignee-zone-v2
...
return await (async () => {
  ...
  async function waitForPlacement(level) {
    ...
    clearRuler();
    const previous = parent.getChildByName?.("add2e-toile-araignee-zone");
    if (previous) previous.destroy({ children: true });
    const g = new PIXI.Graphics();
    g.name = "add2e-toile-araignee-zone";
    g.zIndex = 100000;
    g.eventMode = "none";
    ...
    return await new Promise(resolve => {
      ...
      function onDown(e) { e.preventDefault(); e.stopPropagation(); if (e.button === 2) return cleanup(null); if (e.button !== 0) return; update(e); if (!valid) { ui.notifications.warn(`${SPELL.name} : point d’ancrage hors portée.`); return; } cleanup({ point: current, sideMeters, rangeMeters }, true); }
  ...
  function tokensInSquare(center, sideMeters) {
    const size = metersToPx(sideMeters);
    const x1 = center.x - size / 2, y1 = center.y - size / 2, x2 = center.x + size / 2, y2 = center.y + size / 2;
  ...
  async function playVfx(point, sideMeters) {
    if (typeof Sequence !== "undefined") {
      try { await new Sequence().effect().file("jb2a.web.01.white").atLocation(point).scaleToObject?.(Math.max(1.5, sideMeters / 3)).duration(9000).play(); return true; } catch (err) { console.warn(`${TAG}[VFX_SEQUENCE_FAILED]`, err); }
    }
    return false;
  }

  async function askResults(targets, durationTurns, level) {
    const DialogV2 = foundry?.applications?.api?.DialogV2;
    if (!DialogV2?.wait) { ui.notifications.error(`${SPELL.name} : DialogV2 indisponible.`); return null; }
    const rows = targets.map(t => `<label style="display:grid;grid-template-columns:34px 1fr 132px;gap:8px;align-items:center;border:1px solid #8e63c7;border-radius:7px;background:#fffaff;padding:6px;margin-bottom:5px;"><img src="${esc(t.document?.texture?.src || t.actor?.img || 'icons/svg/mystery-man.svg')}" style="width:32px;height:32px;border-radius:5px;object-fit:cover;"><span style="font-weight:900;color:#2d2144;">${esc(t.name)}</span><select name="target.${esc(t.id)}" style="width:100%;"><option value="trapped">Prise</option><option value="slowed">Ralentie</option><option value="free">Libre</option></select></label>`).join("");
    return await DialogV2.wait({ window: { title: SPELL.name, icon: "fas fa-spider-web" }, modal: true, rejectClose: false, content: `<form style="min-width:450px;max-width:580px;font-family:var(--font-primary);color:#2d2144;"><section style="border:1px solid #8e63c7;border-radius:9px;background:#f6f0ff;padding:8px;margin-bottom:8px;text-align:center;"><b style="color:#6c31b5;text-transform:uppercase;">Toile d’araignée</b><br><span style="font-size:12px;">Cibles détectées dans la zone. Durée : ${durationTurns} tour${durationTurns>1?'s':''}. Taille : ${3 * level} m d’arête.</span></section>${rows}<p style="font-size:12px;color:#4a2e78;margin:.35em 0 0;">La toile doit être ancrée à des points solides. Sans ancrage, le sort échoue selon le MD.</p></form>`, buttons: [{ action: "apply", label: "Appliquer", icon: "fas fa-check", default: true, callback: (_e,_b,dialog) => { const form = dialog.element?.querySelector("form"); const data = new FormData(form); return Object.fromEntries(targets.map(t => [t.id, data.get(`target.${t.id}`) || "trapped"])); } }, { action: "cancel", label: "Annuler", icon: "fas fa-times", callback: () => null }] });
  }

  async function applyWebEffect(targetActor, sourceItem, state, durationRounds) {
  ...
  async function chat(targets, results, durationTurns, level) {
  ...
  let results = {};
  if (targets.length) {
    results = await askResults(targets, durationTurns, level);
    if (!results) return false;
    for (const t of targets) await applyWebEffect(t.actor, sourceItem, results[t.id], durationRounds);
  }
  await chat(targets, results, durationTurns, level);
