 ...
 * Version : 2026-05-21-epouvante-no-fallback-v1
 *
...
console.log("%c[ADD2E][APAISEMENT] 2026-05-21-epouvante-no-fallback-v1", "color:#b88924;font-weight:bold;");
...
const __add2eOnUseResult = await (async () => {
  ...
  function getMovementMeters(actorDoc) {
    ...
    for (const [source, raw] of candidates) {
      ...
      if (Number.isFinite(value)) {
        console.log("[ADD2E][APAISEMENT][EPOUVANTE][MOVE_SOURCE]", {
          actor: actorDoc?.name,
          actorType: actorDoc?.type,
          source,
          raw,
          value
        });
    ...
    console.error("[ADD2E][APAISEMENT][EPOUVANTE][MOVE_MISSING]", {
      actor: actorDoc?.name,
      actorType: actorDoc?.type,
      system: {
        mouvement: sys.mouvement,
        movement: sys.movement,
        movement_raw: sys.movement_raw,
        movement_max: sys.movement_max,
        movement_modes: sys.movement_modes,
        vitesse_deplacement: sys.vitesse_deplacement
      }
    });
  ...
  function fleeDestination(casterDoc, targetDoc, maxMeters) {
    ...
    return {
      x,
      y,
      maxMeters: Math.round(maxMeters * 100) / 100,
      movedMeters: Math.round(movedMeters * 100) / 100,
      from: { x: Number(targetDoc.x ?? 0), y: Number(targetDoc.y ?? 0) },
      grid,
      gridDistance,
      gridUnits
    };
  ...
  async function moveTokenForEpouvante(casterDoc, targetDoc, actorDoc) {
    ...
    if (!move.ok) {
      return {
        ...
        requested: false,
        fatal: true,
        reason: "Mouvement absent ou invalide dans le JSON de la cible.",
        maxMeters: 0,
        movedMeters: 0
      };
    }
    ...
    if (move.value <= 0) {
      return {
        ...
        requested: false,
        fatal: false,
        reason: "La cible a un mouvement de 0 : aucune fuite possible.",
        maxMeters: 0,
        movedMeters: 0
      };
    }
    ...
    console.log("[ADD2E][APAISEMENT][EPOUVANTE][DESTINATION]", {
      actor: actorDoc?.name,
      move,
      dest
    });
    ...
    if (!Number.isFinite(dest.x) || !Number.isFinite(dest.y)) {
      return {
        ...
        requested: false,
        fatal: true,
        reason: "Destination de fuite invalide.",
        maxMeters: move.value,
        movedMeters: 0
      };
    }
    ...
    if (Math.abs(dest.x - Number(targetDoc.x ?? 0)) < 1 && Math.abs(dest.y - Number(targetDoc.y ?? 0)) < 1) {
      return {
        ...
        requested: false,
        fatal: false,
        reason: "La cible ne peut pas être éloignée davantage dans cette direction.",
        ...dest
      };
    }
    ...
    const flagData = {
      ...
      casterTokenName: casterDoc?.name ?? null,
      movedAt: Date.now(),
    ...
    const updateOptions = {
      ...
      add2eForcedMovement: true,
      add2eReason: "epouvante-forced-flee"
    };

    console.log("[ADD2E][APAISEMENT][EPOUVANTE][MOVE_ATTEMPT]", {
      user: game.user.name,
      isGM: game.user.isGM,
      target: targetDoc?.name,
      tokenId: targetDoc?.id,
      updateData,
      updateOptions
    });

    if (game.user.isGM) {
      await targetDoc.update(updateData, updateOptions);
      const after = { x: Number(targetDoc.x ?? 0), y: Number(targetDoc.y ?? 0) };
      const moved = Math.abs(after.x - dest.x) < 1 && Math.abs(after.y - dest.y) < 1;
      console.log("[ADD2E][APAISEMENT][EPOUVANTE][MOVE_APPLIED]", { after, expected: { x: dest.x, y: dest.y }, moved });
      return {
        moved,
        requested: false,
        fatal: !moved,
        reason: moved ? "Déplacement appliqué." : "La mise à jour du token n'a pas abouti.",
        ...dest
      };
    }

    gmRelay("updateToken", {
      sceneId: targetDoc.parent?.id ?? canvas.scene?.id,
      tokenId: targetDoc.id,
      updateData,
    ...
    return {
      ...
      requested: true,
      fatal: false,
      reason: "Déplacement demandé au MJ.",
      ...dest
    };
  }
  ...
  if (mode === "epouvante") {
    ...
    if (moveResult.fatal) {
      ui.notifications.error(`Épouvante : ${moveResult.reason}`);
      return false;
    }
