 ...
 * Clerc niveau 1
 * Version : 2026-06-28-apaisement-source-item-v2
 *
...
console.log("%c[ADD2E][APAISEMENT] 2026-06-28-apaisement-source-item-v2", "color:#b88924;font-weight:bold;");
...
const __add2eOnUseResult = await (async () => {
  ...
  function resolveSpellMode(sourceItem) {
    const directName = norm(sourceItem?.name);
    if (directName === "epouvante" || directName.startsWith("epouvante_")) return "epouvante";
    if (directName === "apaisement" || directName.startsWith("apaisement_")) return "apaisement";
    ...
    for (const candidate of candidates) {
      if (candidate === "epouvante" || candidate.startsWith("epouvante_")) modes.add("epouvante");
      if (candidate === "apaisement" || candidate.startsWith("apaisement_")) modes.add("apaisement");
    }
