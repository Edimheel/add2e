// Sommeil.js - AD&D2
// Correctif V2 : Restauration de la méthode SOCKET pour contourner les permissions.
...
function registerSleepHooks() {
  ...
  const endSleepVfx = (effect) => {
    ...
    for (const token of tokens) {
      ...
      if (typeof Sequencer !== "undefined") Sequencer.EffectManager.endEffects({ name: visName, object: token });
    }
  };
  Hooks.on("deleteActiveEffect", endSleepVfx);
  Hooks.on("updateActiveEffect", (e, c) => { if (c.disabled) endSleepVfx(e); });
}
...
(async () => {
  ...
  if (!sourceItem) { ui.notifications.error("Sort introuvable."); return; }
  const caster = (typeof actor !== "undefined" && actor) ? actor : sourceItem.parent;
  if (!caster) { ui.notifications.error("Lanceur introuvable."); return; }
  ...
  if (hasWarpGate) {
    const cross = await warpgate.crosshairs.show({
      size: 3, icon: sourceItem.img || "icons/svg/sleep.svg",
      label: "Sommeil (3\" diam.)", drawIcon: true, drawOutline: true, interval: 1
    });
    if (!cross || cross.cancelled) { await refund("Annulé."); return; }
  ...
  if (!targets.length) { await refund("Personne dans la zone."); return; }
  ...
  if (!ordered.length) { ui.notifications.warn("Aucune cible valide."); return; }
  ...
  let maxByCat = { "1": n1, "2": n2, "3": n3, "4": n4, "4+": 1, "HIGH": 0 };
  let count = { "1": 0, "2": 0, "3": 0, "4": 0, "4+": 0, "HIGH": 0 };
  ...
  for (let entry of ordered) {
    ...
    let isImmune = cible.effects.some(e => (e.label || e.name || "").toLowerCase().includes("immunité"));
    
    if (cat === "HIGH") { status = `Trop Puissant (${dv} DV)`; color = "#7f8c8d"; }
    else if (isImmune) { status = "Immunisé"; color = "#7f8c8d"; }
    else if (count[cat] >= maxByCat[cat]) { status = "Épargné (Quota)"; color = "#2980b9"; }
    else {
      count[cat]++;
  ...
  const rows = listResults.map(r => `
    <div style="display:flex;justify-content:space-between;border-bottom:1px dashed #eee;font-size:0.9em;">
        <span>${r.name}</span><span style="font-weight:bold;color:${r.color};">${r.status}</span>
    </div>`).join("");
  ...
  <div class="add2e-spell-card" style="border-radius:12px;box-shadow:0 4px 10px #715aab44;background:linear-gradient(135deg,#fdfbfd 0%,#f4efff 100%);border:1.5px solid #9373c7;overflow:hidden;padding:0;">
    <div style="background:linear-gradient(90deg,#6a3c99 0%,#8e44ad 100%);padding:8px;color:white;display:flex;align-items:center;gap:10px;">
        ...
        <div style="line-height:1.2;flex:1;"><div style="font-weight:bold;">${caster.name}</div><div style="font-size:0.8em;opacity:0.9;">lance ${sourceItem.name}</div></div>
  ...
  ChatMessage.create({ speaker: ChatMessage.getSpeaker({ actor: caster }), content: chatHtml });

  // ==============================================================
  // 8. APPLICATION EFFETS (VIA SOCKET SYSTEME RESTAURÉ)
  // ==============================================================
  let jb2aPath = null;
  if (game.modules.get("jb2a_patreon")?.active) jb2aPath = "modules/jb2a_patreon/Library/1st_Level/Sleep/Cloud01_01_Dark_OrangePurple_400x400.webm";
  else if (game.modules.get("jb2a_free")?.active) jb2aPath = "modules/jb2a_free/Library/1st_Level/Sleep/Cloud01_01_Dark_OrangePurple_400x400.webm";
  ...
  for (let t of affectedTokens) {
      ...
      const effectData = {
          ...
          duration: { rounds: 5 },
          flags: { add2e: { tags: ["etat:sommeil"] } },
          origin: sourceItem.uuid
      };

      // [CRUCIAL] On utilise le socket du système pour contourner les permissions
      if (game.socket) {
      ...
      if (jb2aPath && typeof Sequence !== "undefined") {
          new Sequence()
            .effect().file(jb2aPath).attachTo(t).persist(true).name(`sleep-effect-${t.id}`)
            .belowTokens(true).scale(0.5).opacity(0.6).play().catch(e=>{});
      }
  }
