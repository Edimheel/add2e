/** ADD2E — Langage animal — Clerc niveau 2 — DialogV2 + VFX — V13/V14/V15 */
const __add2eOnUseResult = await (async () => {
  const SPELL = {"id":"langage-animal","name":"Langage animal","fx":"communication","color":"#91d36b","min":1,"max":1};
  const RULES = ["Durée : 2 rounds par niveau.","Cible : un animal traditionnel doté d’un esprit, pas un monstre.","Le sort permet questions, réponses et compréhension.","Le MJ détermine toute faveur ou service avec la réaction, le charisme et le comportement du clerc."];
  ...
  const details = [];
  details.push(`Durée : <b>${2 * level} rounds</b>.`);
...
if (__add2eOnUseResult !== true && __add2eOnUseResult !== false) { console.error("[ADD2E][ONUSE][BAD_RETURN_STRICT]", { script: "langage-des-animaux.js", result: __add2eOnUseResult }); return false; }
