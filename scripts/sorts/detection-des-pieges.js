/** ADD2E — Détection des pièges — Clerc niveau 2 — DialogV2 + VFX — V13/V14/V15 */
const __add2eOnUseResult = await (async () => {
  const SPELL = {"id":"detection-pieges","name":"Détection des pièges","fx":"detection","color":"#ffd36a","min":0,"max":0};
  const RULES = ["Détection directionnelle : le clerc doit faire face à la direction examinée.","Révèle les pièges mécaniques ou magiques, normalement ou magiquement cachés ; ne les désamorce pas.","Pour un piège magique ou caché par magie : 10 % par niveau pour identifier le type de magie.","Le MJ confirme les pièges présents et leur nature."];
  ...
  const details = [];
  details.push(`Chance d’identifier le type de magie : <b>${10 * level} %</b>.`);
...
if (__add2eOnUseResult !== true && __add2eOnUseResult !== false) { console.error("[ADD2E][ONUSE][BAD_RETURN_STRICT]", { script: "detection-des-pieges.js", result: __add2eOnUseResult }); return false; }
