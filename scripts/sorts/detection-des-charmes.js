/** ADD2E — Détection des charmes — Clerc niveau 2 — DialogV2 + VFX — V13/V14/V15 */
const __add2eOnUseResult = await (async () => {
  const SPELL = {"id":"detection-charmes","name":"Détection des charmes","fx":"detection","color":"#8fd3ff","min":1,"max":10};
  const RULES = ["Durée : 1 tour ; jusqu’à 10 créatures peuvent être examinées.","Le jet de protection annule la détection.","La forme inverse protège une seule créature.","Aucun état de charme n’est deviné ou révélé automatiquement."];
...
if (__add2eOnUseResult !== true && __add2eOnUseResult !== false) { console.error("[ADD2E][ONUSE][BAD_RETURN_STRICT]", { script: "detection-des-charmes.js", result: __add2eOnUseResult }); return false; }
