...
const __add2eOnUseResult = await (async () => {
  ...
  const reverseName = isGood ? "Détection du mal" : "Détection du bien";
  await ChatMessage.create({
    speaker: ChatMessage.getSpeaker({ actor: caster }),
    content: `<div class="add2e-spell-card add2e-spell-card-clerc" style="border-radius:12px;box-shadow:0 4px 10px #0002;background:linear-gradient(135deg,#fffaf0 0%,#fff7df 100%);border:1.5px solid #e2bc63;overflow:hidden;padding:0;font-family:var(--font-primary);"><div style="background:linear-gradient(90deg,#6f4b12 0%,#b88924 100%);padding:8px 12px;color:white;display:flex;align-items:center;gap:10px;border-bottom:2px solid #8a611d;"><img src="${escapeHtml(caster?.img || "icons/svg/mystery-man.svg")}" style="width:36px;height:36px;border-radius:50%;border:2px solid #fff;object-fit:cover;"><div style="line-height:1.2;flex:1;"><div style="font-weight:bold;font-size:1.05em;">${escapeHtml(caster?.name ?? "Lanceur")}</div><div style="font-size:0.85em;opacity:0.95;">lance <b>${escapeHtml(spellName)}</b></div></div><div style="text-align:right;font-size:0.78em;opacity:0.95;">Sort divin</div><img src="${escapeHtml(sourceItem?.img || "systems/add2e/assets/icones/sorts/detection-du-mal.webp")}" style="width:32px;height:32px;border-radius:4px;background:#fff;"></div><div style="padding:10px;"><div style="margin-bottom:6px;font-size:0.95em;color:#6f4b12;"><b>Cible :</b> devant le lanceur</div><div style="border:1px solid #e2bc63;background:#fffdf4;border-radius:6px;padding:8px;color:#6f4b12;"><div style="text-align:center;font-weight:bold;color:${matches.length ? "#2f8f46" : "#b88924"};">${title}</div><div><b>Direction / zone :</b> devant le lanceur</div><div><b>Durée :</b> ${rounds} round(s).</div><ul style="margin:6px 0 0 18px;">${rows}</ul></div><details style="margin-top:8px;background:white;border:1px solid #e2bc63;border-radius:6px;"><summary style="cursor:pointer;color:#6f4b12;font-weight:600;padding:6px;">Règle appliquée</summary><div style="padding:8px;font-size:0.85em;line-height:1.45;color:#6f4b12;"><div>Le sort détecte les émanations du ${sideLabel} dans une bande de 12&quot; de long et 1&quot; de large, devant le lanceur.</div><div>Il révèle le degré et la nature générale de l’émanation, sans identifier complètement la cible.</div><div>Version inverse : <b>${reverseName}</b>.</div></div></details></div></div>`,
