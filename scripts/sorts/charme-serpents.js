/** ADD2E — Charme-serpents — Clerc niveau 2 — DialogV2 + VFX — V13/V14/V15 */
const __add2eOnUseResult = await (async () => {
  const SPELL = {"id":"charme-serpents","name":"Charme-serpents","fx":"charme","color":"#66d17a","min":1,"max":99};
  const RULES = ["Le total des points de vie des serpents ne peut pas dépasser ceux du clerc.","Torpeur : 1d4+2 tours ; éveillés et calmes : 1 à 3 tours ; agressifs ou affamés : 1d4+4 rounds.","Ophidiens assimilés : résistance à la magie et jets de protection restent applicables.","Le MJ confirme l’état, les points de vie et la durée de chaque cible."];
  ...
  const details = [];
  details.push(`Serpents / ophidiens sélectionnés : <b>${targets.length}</b>.`);
...
if (__add2eOnUseResult !== true && __add2eOnUseResult !== false) { console.error("[ADD2E][ONUSE][BAD_RETURN_STRICT]", { script: "charme-serpents.js", result: __add2eOnUseResult }); return false; }
