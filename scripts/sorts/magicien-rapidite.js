// ADD2E — onUse Magicien : Rapidité
// Compatible Foundry V13/V14/V15.
// Effet de mouvement résolu par le domaine canonique movement.

const ADD2E_SORT_VERSION = "2026-07-30-rapidite-canonical-movement-v1";
const ADD2E_SORT_TAG = "[ADD2E][SORT_ONUSE][MAGICIEN][RAPIDITE]";
...
async function add2eApplySpeedEffect(targetActor, rounds) {
  ...
  const ownTag = "sort:rapidite";
  const oppositeTag = "sort:ralentissement";
  ...
  const sourceId = String(ADD2E_CONTEXT_ITEM?.id ?? "rapidite");
  const sourceUuid = String(ADD2E_CONTEXT_ITEM?.uuid ?? "");
  await targetActor.createEmbeddedDocuments("ActiveEffect", [{
    name: ADD2E_CONTEXT_ITEM?.name ?? "Rapidité",
    ...
    description: "La vitesse de déplacement et le nombre d’attaques sont doublés. Le temps d’incantation n’est pas modifié.",
    flags: {
      add2e: {
        ...
        modifiers: [{
          id: `spell:${sourceId}:rapidite:movement-ground`,
          domain: "movement",
          target: "ground",
          operation: "multiply",
          value: 2,
...
async function add2eCreateResultCard(caster, results, rounds) {
  ...
  const rows = [
    ...
    { label: "Mouvement", value: "×2" },
    { label: "Effet", value: "double aussi le nombre d’attaques" }
  ];
  if (applied.length) rows.push({ label: "Effet appliqué", value: applied.join(", ") });
  if (cancelled.length) rows.push({ label: "Ralentissement annulé", value: cancelled.join(", ") });
  rows.push({ label: "Contrecoup", value: "Vieillissement magique d’un an par créature affectée" });

  const options = {
    actor: caster,
    title: ADD2E_CONTEXT_ITEM?.name ?? "Rapidité",
    ...
    rows,
    message: "La vitesse de déplacement et le nombre d’attaques sont doublés. Le temps d’incantation n’est pas modifié.",
    chatData: {
      flags: {
        add2e: {
          spellEffect: "rapidite",
          version: ADD2E_SORT_VERSION
        }
      }
    }
...
if (!caster) {
  ui.notifications.error("Rapidité : lanceur introuvable.");
  return false;
}
...
if (!targets.length) {
  ui.notifications.warn("Rapidité : aucune cible disponible.");
  return false;
}
if (targets.length > level) {
  ui.notifications.warn(`Rapidité peut affecter au maximum ${level} créature(s) au niveau ${level}.`);
  return false;
}
