// ADD2E — Rappel à la vie : résolution canonique de résurrection.
// Compatible Foundry V13/V14/V15. Aucun Dialog historique.

const targets = Array.from(game.user?.targets ?? []);
if (targets.length !== 1) {
  ui.notifications.warn("Rappel à la vie : cible unique obligatoire.");
  return false;
}

const targetActor = targets[0]?.actor ?? null;
if (!targetActor) {
  ui.notifications.warn("Rappel à la vie : acteur cible introuvable.");
  return false;
}
...
if (!result?.attempted) {
  ...
  ui.notifications.warn(`Rappel à la vie : ${messages[result?.reason] ?? "la tentative est impossible."}`);
  return false;
}
