...
// Version : 2026-05-28-groupe-a-cone-detection-v4
...
return await (async () => {
  ...
  function effectiveConeDistanceMeters() {
    return Math.max(SPELL.coneDistanceMeters, metersPerGridCell() * 1.05);
  }

  function pxDistanceToMeters(px) {
  ...
  function bearingFromSourceToPoint(sourceToken, point) {
    ...
    const radians = Math.atan2(point.y - sy, point.x - sx);
  ...
  function pointInCone(sourceToken, point, directionDeg) {
    const sx = sourceToken.center?.x ?? (sourceToken.document.x + sourceToken.w / 2);
    const sy = sourceToken.center?.y ?? (sourceToken.document.y + sourceToken.h / 2);
    const distanceMeters = pxDistanceToMeters(Math.hypot(point.x - sx, point.y - sy));
    if (distanceMeters > effectiveConeDistanceMeters()) return false;
    return angleDiffDegrees(directionDeg, bearingFromSourceToPoint(sourceToken, point)) <= SPELL.coneAngle / 2;
  }
  ...
  function tokensInCone(sourceToken, directionDeg) {
    return canvas.tokens.placeables
      ...
      .filter(t => tokenSamplePoints(t).some(point => pointInCone(sourceToken, point, directionDeg)));
  }
  ...
  function coneRadiusPx() {
    return effectiveConeDistanceMeters() / metersPerGridCell() * gridSizePx();
  }
  ...
  async function waitForConePlacement(sourceToken) {
    ...
    return await new Promise(resolve => {
      ...
      const updateDirectionFromEvent = event => {
        ...
        const cx = sourceToken.center?.x ?? (sourceToken.document.x + sourceToken.w / 2);
        const cy = sourceToken.center?.y ?? (sourceToken.document.y + sourceToken.h / 2);
        direction = canvasRadiansToFoundryRotation(Math.atan2(point.y - cy, point.x - cx));
        drawConeGraphic(overlay, sourceToken, direction);
      };

      function onMouseMove(event) { updateDirectionFromEvent(event); }
  ...
  async function createChat({ caster, sourceItem, sourceToken, affected, damage, placement }) {
    ...
    const rows = affected.length
      ...
      : `<tr><td colspan="2" style="padding:6px;text-align:center;"><i>Aucune créature détectée dans le cône.</i></td></tr>`;

    await ChatMessage.create({
      speaker: ChatMessage.getSpeaker({ actor: caster, token: sourceToken }),
      content: `
        <div class="add2e-chat-card add2e-magicien-sort add2e-sort-mains-brulantes" style="border:1px solid #8e63c7;border-radius:8px;overflow:hidden;background:#f6f0ff;color:#2d2144;font-family:var(--font-primary);">
          ...
          <div style="padding:9px 10px 10px 10px;background:#f6f0ff;">
            <div style="border:1px solid #8e63c7;border-radius:6px;background:#fffaff;padding:8px;margin-bottom:7px;">
              ...
              <table style="width:100%;border-collapse:collapse;margin-top:6px;font-size:13px;"><thead><tr><th style="text-align:left;padding:4px 6px;">Créature dans le cône</th><th style="text-align:right;padding:4px 6px;">Dégâts</th></tr></thead><tbody>${rows}</tbody></table>
            </div>
            <details style="border:1px solid #8e63c7;border-radius:5px;background:#fffaff;padding:5px 7px;margin-top:7px;">
              ...
              <div style="margin-top:5px;font-size:12px;line-height:1.35;">
                ...
                <p><b>Composantes :</b> ${esc(SPELL.componentsText)} — <b>Incantation :</b> ${esc(SPELL.castingTimeText)}.</p>
  ...
  console.log(`${TAG}[START]`, {
    ...
    effectiveConeDistanceMeters: effectiveConeDistanceMeters(),
