 ...
 * ADD2E — Sanctuaire
 * Clerc niveau 1
 * Version : 2026-06-06-sanctuaire-time-engine-v1
 *
 * Contrat onUse : true = consommé ; false = non consommé.
 */

console.log("%c[ADD2E][SANCTUAIRE] 2026-06-06-sanctuaire-time-engine-v1", "color:#b88924;font-weight:bold;");
...
const __add2eOnUseResult = await (async () => {
  ...
  function roundDuration(level) {
    ...
    return time?.toRounds?.("2+level", "round", { level }) ?? (2 + Math.max(1, Number(level) || 1));
  }
  ...
  function effectData({ sourceItem, caster, targetActor, level }) {
    ...
    return {
      name: "Sanctuaire",
      img: sourceItem?.img || "systems/add2e/assets/icones/sorts/sanctuaire.webp",
      origin: sourceItem?.uuid ?? null,
      disabled: false,
      transfer: false,
      duration: durationData(rounds),
      description: `Les adversaires qui veulent attaquer la créature protégée doivent réussir un jet de protection. La protection cesse si le bénéficiaire entreprend une action offensive. Durée : ${rounds} rounds.`,
      flags: {
        add2e: {
  ...
  async function applyEffect(targetActor, data) {
    ...
    if (game.user.isGM || targetActor.isOwner) {
      const oldIds = Array.from(targetActor.effects ?? [])
        .filter(e => (e.flags?.add2e?.tags ?? []).includes("sort:sanctuaire"))
        .map(e => e.id)
        .filter(Boolean);
    ...
    if (!emitted) {
      ui.notifications.error("Sanctuaire : socket indisponible, impossible de demander l’effet au MJ.");
      return false;
    }
        ...
        <div class="add2e-spell-card add2e-spell-card-clerc" style="border-radius:12px;box-shadow:0 4px 10px #0002;background:linear-gradient(135deg,#fffaf0 0%,#fff7df 100%);border:1.5px solid #e2bc63;overflow:hidden;padding:0;font-family:var(--font-primary);">
          <div style="background:linear-gradient(90deg,#6f4b12 0%,#b88924 100%);padding:8px 12px;color:white;display:flex;align-items:center;gap:10px;border-bottom:2px solid #8a611d;">
            ...
            <div style="line-height:1.2;flex:1;">
              ...
              <div style="font-size:0.85em;opacity:0.95;">lance <b>${esc(sourceItem?.name ?? "Sanctuaire")}</b></div>
            </div>
            <div style="text-align:right;font-size:0.78em;opacity:0.95;">Sort divin</div>
            <img src="${esc(sourceItem?.img || "systems/add2e/assets/icones/sorts/sanctuaire.webp")}" style="width:32px;height:32px;border-radius:4px;background:#fff;">
          </div>
          <div style="padding:10px;">
            ...
            <div style="border:1px solid #e2bc63;background:#fffdf4;border-radius:6px;padding:8px;text-align:center;color:#6f4b12;">
              <div style="font-weight:bold;color:#2f8f46;">SANCTUAIRE APPLIQUÉ</div>
              <div>Durée : <b>${esc(rounds)} rounds</b>.</div>
              <div>Les adversaires doivent réussir un jet de protection pour attaquer la cible.</div>
            </div>
            <details style="margin-top:8px;background:white;border:1px solid #e2bc63;border-radius:6px;">
              ...
              <div style="padding:8px;font-size:0.85em;line-height:1.45;color:#6f4b12;">
                Les adversaires qui veulent attaquer la créature protégée doivent réussir un jet de protection. La protection cesse si le bénéficiaire entreprend une action offensive.
              </div>
            </details>
          </div>
        </div>`,
  ...
  try {
    ...
    await globalThis.ADD2E_PLAY_SPELL_FX?.("sanctuaire", { casterToken, targetToken });
  } catch (err) {
    console.warn("[ADD2E][SANCTUAIRE][VFX][IGNORED]", err);
  }

  await createChat({ caster, sourceItem, targetToken, rounds });

  console.log("[ADD2E][sanctuaire.js][ONUSE_RESULT]", true);
  return true;
})();

if (__add2eOnUseResult !== true && __add2eOnUseResult !== false) {
  console.error("[ADD2E][ONUSE][BAD_RETURN_STRICT] Le script onUse doit retourner true ou false.", { script: "sanctuaire.js", result: __add2eOnUseResult });
  ui.notifications?.error?.("Sanctuaire : le script onUse n'a pas retourné true/false.");
  return false;
}
