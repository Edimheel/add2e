/** ADD2E — Cantique — Clerc niveau 2 — DialogV2 + VFX — V13/V14/V15 */
const __add2eOnUseResult = await (async () => {
  const SPELL = {"id":"cantique","name":"Cantique","fx":"cantique","color":"#ffd76a","min":0,"max":99};
  const RULES = ["Zone : rayon de 3 pouces autour du clerc.","Alliés : +1 aux attaques, dommages et jets de protection.","Ennemis : −1 aux mêmes jets.","L’effet cesse dès que le clerc ne chante plus, se déplace ou est interrompu."];
  ...
  const details = [];
  details.push(`Cibles sélectionnées pour le rappel : <b>${targets.length}</b>.`);
...
if (__add2eOnUseResult !== true && __add2eOnUseResult !== false) { console.error("[ADD2E][ONUSE][BAD_RETURN_STRICT]", { script: "cantique.js", result: __add2eOnUseResult }); return false; }
