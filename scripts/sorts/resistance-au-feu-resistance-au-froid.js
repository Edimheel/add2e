/** ADD2E — Résistance au feu — Clerc niveau 2 — DialogV2 + VFX — V13/V14/V15 */
const __add2eOnUseResult = await (async () => {
  const SPELL = {"id":"resistance-feu","name":"Résistance au feu","fx":"resistance_feu","color":"#ff8a54","min":1,"max":1};
  const RULES = ["Durée : 1 tour par niveau sur la créature touchée.","Bonus de +3 au jet de protection contre le feu.","Dégâts de feu : quart si la sauvegarde réussit, moitié si elle échoue.","La goutte de mercure est consommée.","La réduction des dégâts reste au MJ faute de mécanisme centralisé confirmé."];
  ...
  const details = [];
  details.push(`Durée : <b>${level} tour(s)</b>.`);
...
if (__add2eOnUseResult !== true && __add2eOnUseResult !== false) { console.error("[ADD2E][ONUSE][BAD_RETURN_STRICT]", { script: "resistance-au-feu-resistance-au-froid.js", result: __add2eOnUseResult }); return false; }
