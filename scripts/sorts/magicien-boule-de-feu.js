...
const __add2eFireballResult = await (async () => {
  const VERSION = "2026-08-12-canonical-fireball-v2";
  const DAMAGE_TYPE = "feu";

  if (typeof globalThis.add2eRollSavingThrow !== "function") {
    ui.notifications?.error?.("Boule de feu : le résolveur canonique des jets de sauvegarde est indisponible.");
    return false;
  }
  if (typeof globalThis.add2eApplyDamage !== "function") {
    ui.notifications?.error?.("Boule de feu : le résolveur canonique des dégâts est indisponible.");
    return false;
  }
  if (typeof globalThis.add2eBuildChatCard !== "function" || typeof globalThis.add2eCreateChatCard !== "function") {
    ui.notifications?.error?.("Boule de feu : les constructeurs communs de cartes ADD2E sont indisponibles.");
    return false;
  }
  ...
  if (!sourceItem || String(sourceItem.type ?? "").toLowerCase() !== "sort") {
    ui.notifications?.error?.("Boule de feu : Item sort introuvable.");
    return false;
  }

  const caster = (typeof actor !== "undefined" && actor) ? actor : sourceItem.parent;
  if (!caster) {
    ui.notifications?.error?.("Boule de feu : lanceur introuvable.");
    return false;
  }
  ...
  const casterToken = (typeof token !== "undefined" && token?.actor?.id === caster.id)
    ...
    : canvas.tokens?.controlled?.find(placeable => placeable?.actor?.id === caster.id)
      ...
      ?? null;
  if (!casterToken) {
    ui.notifications?.warn?.("Boule de feu : sélectionne le token du lanceur.");
    return false;
  }
  ...
  const resolveCasterLevel = () => {
    if (sourceItem.system?.isObjectPower === true) {
      ...
      if (!Number.isInteger(explicit) || explicit < 1) {
        throw new Error("Boule de feu : niveau de lanceur explicite absent du pouvoir d’objet magique.");
      }
      return explicit;
    }
    const resolver = globalThis.add2eCanActorUseSpell;
    if (typeof resolver !== "function") {
      throw new Error("Boule de feu : le résolveur canonique de lancement des sorts est indisponible.");
    }
    ...
    if (access?.ok !== true || !Number.isInteger(actorLevel) || actorLevel < 1) {
      throw new Error(`Boule de feu : niveau canonique du lanceur indisponible${access?.reason ? ` (${access.reason})` : ""}.`);
    }
    return actorLevel;
  };
  ...
  try {
    ...
    ui.notifications?.error?.(error?.message ?? "Boule de feu : niveau du lanceur indisponible.");
    return false;
  }
  ...
  if (!targets.length) {
    ui.notifications?.warn?.("Boule de feu : cible les créatures prises dans l’explosion.");
    return false;
  }
  ...
  for (const targetToken of targets) {
    ...
    if (!save?.ok) {
      ui.notifications?.error?.(`Boule de feu : jet de sauvegarde indisponible pour ${targetToken.name ?? targetToken.actor.name}.`);
      return false;
    }
  ...
  for (const { targetToken, save } of saveResults) {
    const resolution = await globalThis.add2eApplyDamage({
      ...
      type: DAMAGE_TYPE,
      details: `${sourceItem.name ?? "Boule de feu"} — explosion de feu`,
      sourceItem,
      lanceur: caster,
      save: {
      ...
      actionTags: ["spell", "damage:feu", "spell:boule_de_feu"]
    });
    const damage = Number(resolution?.amount);
    if (!resolution?.applied || !Number.isFinite(damage)) {
      ui.notifications?.error?.(`Boule de feu : impossible d’appliquer les dégâts à ${targetToken.name ?? targetToken.actor.name}.`);
      return false;
    }
  ...
  const card = {
    actor: caster,
    title: sourceItem.name ?? "Boule de feu",
    icon: "fas fa-fire",
    variant: "damage",
    source: {
      name: caster.name,
    ...
    target: {
      ...
      type: "Créatures dans l’explosion",
      meta: ""
    },
    rows,
  ...
  if (!String(preview ?? "").trim()) throw new Error("Boule de feu : carte de chat vide.");
  await globalThis.add2eCreateChatCard(card);
  return true;
})();

return __add2eFireballResult === true ? true : false;
