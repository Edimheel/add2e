/** ADD2E — Augure — Clerc niveau 2 — DialogV2 + VFX — V13/V14/V15 */
const __add2eOnUseResult = await (async () => {
  const SPELL = {"id":"augure","name":"Augure","fx":"augure","color":"#b78cff","min":0,"max":0};
  const RULES = ["Futur immédiat : 3 tours maximum.","Chance correcte : 70 % + 1 % par niveau du clerc.","Le MJ détermine les ajustements et formule la réponse ; aucune réponse n’est automatisée.","Composantes alternatives : objets divinatoires, ou infusion avec perle écrasée d’au moins 100 po."];
  ...
  const details = [];
  details.push(`Chance de base affichée : <b>${70 + level} %</b>.`);
...
if (__add2eOnUseResult !== true && __add2eOnUseResult !== false) { console.error("[ADD2E][ONUSE][BAD_RETURN_STRICT]", { script: "augure.js", result: __add2eOnUseResult }); return false; }
