...
return await (async () => {
  const VERSION = "2026-08-13-illusionist-dancing-lights-v3";
  const SPELL_KEY = "lumieres_dansantes";
  const EXPECTED_LIST = "illusionniste";
  const LIST_LABEL = "Illusionniste";
  ...
  const card = {
    ...
    source: {
      ...
      type: isObjectPower ? "Pouvoir d’objet magique" : `Sort d’${LIST_LABEL.toLowerCase()}`,
