...
// Le sort conserve son comportement actuel : saisir les paramètres du symbole puis publier une carte commune.

const ADD2E_SYMBOLE_VERSION = "2026-08-09-canonical-note-spell-v4";
const ADD2E_SYMBOLE_CONFIG = Object.freeze({
  name: "Symbole",
  slug: "symbole",
  level: 7,
  description: "Symbole applique une protection ou une dissipation divine. L’effet et ses conditions sont notés pour être résolus selon la scène et l’arbitrage du MD."
});
...
async function add2eSymboleParameters() {
  ...
  return globalThis.add2eDialogWait({
    ...
    window: { title: ADD2E_SYMBOLE_CONFIG.name },
    content: `
      <form class="add2e-symbole-form">
        <p><b>${add2eSymboleEscape(ADD2E_SYMBOLE_CONFIG.name)}</b></p>
    ...
    buttons: [
      {
        action: "cast",
        label: ADD2E_SYMBOLE_CONFIG.name,
...
async function add2eSymboleChat(caster, casterToken, note) {
  ...
  const safeDescription = add2eSymboleEscape(ADD2E_SYMBOLE_CONFIG.description);
  const safeNote = add2eSymboleEscape(note);
  const options = {
    actor: caster,
    title: ADD2E_SYMBOLE_CONFIG.name,
    ...
    rows: [
      ...
      { label: "Résultat", value: ADD2E_SYMBOLE_CONFIG.name.toUpperCase() }
    ],
    ...
    chatData: {
      ...
      flags: { add2e: { spell: ADD2E_SYMBOLE_CONFIG.slug, version: ADD2E_SYMBOLE_VERSION } }
    }
  };
...
if (!caster) {
  ui.notifications.error("Symbole : lanceur introuvable.");
  return false;
}

const parameters = await add2eSymboleParameters();
if (!parameters) {
  ui.notifications.info("Symbole annulé.");
  return false;
}

await add2eSymboleChat(caster, add2eSymboleCasterToken(), parameters.note);
return true;
