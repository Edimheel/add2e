/** ADD2E — Paralysie — Clerc niveau 2 — DialogV2 + VFX — V13/V14/V15 */
const __add2eOnUseResult = await (async () => {
  const SPELL = {"id":"paralysie","name":"Paralysie","fx":"paralysie","color":"#79d7ff","min":1,"max":3};
  const RULES = ["Affecte 1 à 3 humains ou humanoïdes bipèdes P-M admissibles.","Durée : 4 rounds + 1 round par niveau.","Sauvegarde normale contre 3 cibles, −1 contre 2, −2 contre 1 ; une réussite annule.","La tige de métal droite et rigide est consommée.","Les sauvegardes et l’état paralysé ne sont pas automatisés sans mécanisme système fiable."];
  ...
  const details = [];
  const penalty = targets.length === 1 ? -2 : targets.length === 2 ? -1 : 0; details.push(`Durée : <b>${4 + level} rounds</b> ; modificateur de sauvegarde : <b>${penalty}</b>.`);
...
if (__add2eOnUseResult !== true && __add2eOnUseResult !== false) { console.error("[ADD2E][ONUSE][BAD_RETURN_STRICT]", { script: "paralysie.js", result: __add2eOnUseResult }); return false; }
