 ...
 * Foundry V13/V14
 * Version : 2026-05-22-v3-stable-vfx-isolated
 */

console.log("%c[ADD2E][AQUAGENESE] SCRIPT CUSTOM CLERC v3", "color:#b88924;font-weight:bold;");
...
const __add2eOnUseResult = await (async () => {
  try {
    ...
    function add2eGetCaster(src) {
      if (typeof actor !== "undefined" && actor) return actor;
      return src?.parent ?? null;
    }

    async function add2eSafePlayFx(mode, casterToken) {
      try {
        ...
        console.warn("[ADD2E][AQUAGENESE][VFX][IGNORED]", e);
      }
    }
    ...
    function add2eClercCard({ caster, sourceItem, mode, nbOutres, litres, maxLitres, itemCreated, itemUpdated }) {
      ...
      let itemHtml = "";
      if (itemCreated) itemHtml = `<div style="margin-top:6px;color:${ADD2E_CLERIC_CHAT.dark};">Équipement créé : <b>${add2eEscapeHtml(itemCreated.name)}</b> × ${nbOutres}</div>`;
      else if (itemUpdated) itemHtml = `<div style="margin-top:6px;color:${ADD2E_CLERIC_CHAT.dark};">Équipement mis à jour : <b>${add2eEscapeHtml(itemUpdated.name)}</b> +${nbOutres}</div>`;
    ...
    const source = add2eGetSourceItem();
    if (!source) { ui.notifications.error("Aquagenèse : sort introuvable."); return false; }

    const caster = add2eGetCaster(source);
    ...
    const dialogResult = await DialogV2.wait({
      ...
      add2eTheme: "cleric",
      add2eImg: source.img || "icons/magic/water/orb-water-blue.webp",
      ...
      try {
        if (existing) {
          ...
          const created = await caster.createEmbeddedDocuments("Item", [{
            ...
            flags: { add2e: { createdBySpell: "Aquagenèse", spellUuid: source.uuid ?? null, casterUuid: caster.uuid ?? null, volumeUnitaireLitres: 5, quantityAdded: nbOutres, litresAdded: litres, createdAt: Date.now() } }
          }]);
    ...
    await add2eSafePlayFx(mode, casterToken);

    await ChatMessage.create({
      speaker: ChatMessage.getSpeaker({ actor: caster }),
      content: add2eClercCard({ caster, sourceItem: source, mode, nbOutres, litres, maxLitres, itemCreated, itemUpdated }),
      ...chatStyleData()
    });

    console.log("[ADD2E][aquagenese.js][ONUSE_RESULT]", true);
    return true;
  } catch (e) {
    console.error("[ADD2E][AQUAGENESE][FATAL]", e);
    ui.notifications.error("Aquagenèse : erreur dans le script. Voir console.");
    return false;
  }
})();
