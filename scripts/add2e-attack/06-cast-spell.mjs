...
export async function add2eCastSpell({ actor, sort } = {}) {
  ...
  async function add2eRefreshActorSpellSheets(actorDoc, sortDoc, value) {
    ...
    // Ne jamais utiliser actorDoc.sheet.render ici :
    // depuis le HUD, Foundry instancie/ouvre la feuille si elle est fermée.
    // On ne rafraîchit que les fenêtres réellement ouvertes.
    try {
      for (const app of Object.values(ui.windows ?? {})) {
        const doc = app?.actor ?? app?.document ?? app?.object ?? null;
