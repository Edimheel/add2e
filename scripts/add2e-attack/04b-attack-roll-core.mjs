...
export const ADD2E_ATTACK_ROLL_CORE_VERSION = "2026-07-09-normal-combat-resolved-vfx-v1";
...
async function add2ePlayResolvedWeaponVfx({ actor, weapon, sourceToken, targetToken } = {}) {
  ...
  try {
    ...
    console.warn("[ADD2E][JB2A][WEAPON][RESOLVED_ATTACK_VFX_ERROR]", { actor: actor?.name, weapon: weapon?.name, error });
    return false;
  }
}
...
async function add2eAttackRollWithResolvedWeaponVfx(...args) {
  ...
  if (result === true) {
    await add2ePlayResolvedWeaponVfx({
      ...
      weapon,
      sourceToken,
      targetToken: payload.targetToken ?? payload.cibleToken ?? targetTokenBeforeRoll ?? Array.from(game.user?.targets ?? [])[0] ?? null
    });
  }
  return result;
}
...
Object.defineProperties(add2eAttackRollWithResolvedWeaponVfx, {
  __add2eWeaponFxWrapped: { value: true, configurable: true },
