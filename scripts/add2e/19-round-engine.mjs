...
function vadeStateSnapshot(state) {
...
async function runPlayerVadeRetroContinuation(payload = {}) {
  ...
  try {
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      ...
      if (combatReady && (stateReady || payloadReady)) {
        combat = candidateCombat;
        state = stateReady ? candidateState : payloadVadeState;
        usedPayloadVadeState = !stateReady;
        break;
      }
      if (attempt + 1 < attempts) await delay(VADE_RETRO_SYNC_RETRY_DELAY_MS);
    }
    ...
    contexts.set(contextKey, {
      ...
      vadeState: clonePlain(state)
    });
    return (await execute(actor, feature, null)) !== false;
  } catch (err) {
    error("[VADE_RETRO_PLAYER_CONTINUATION_ERROR]", { combatId: expectedCombatId, round: expectedRound, err });
    return false;
  } finally {
    if (contexts && contextKey) contexts.delete(contextKey);
    LOCAL_VADE_CONTINUATIONS.delete(requestKey);
  }
}

async function continueVadeRetroOnInitiatingPlayer(combat, currentRound) {
  if (!combat || game.user?.isGM || currentRound <= 0) return false;
  const requestPrefix = `local-vade:${combat.id}:${currentRound}:`;
  const attempts = 6;
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    let found = false;
    for (const { actor } of uniqueCombatActors(combat)) {
      ...
      if (String(state.initiatorUserId ?? "") !== String(game.user?.id ?? "") || actor.isOwner === false) continue;
      found = true;
      const requestId = `${requestPrefix}${actor.uuid ?? actor.id}`;
      const done = await runPlayerVadeRetroContinuation({
        requestId,
        ...
        combatId: combat.id,
        round: currentRound,
        vadeState: vadeStateSnapshot(state)
      });
      if (done) return true;
    }
    if (found || attempt + 1 >= attempts) break;
    await delay(VADE_RETRO_SYNC_RETRY_DELAY_MS);
  }
  return false;
}
...
async function continuePendingVadeRetro(combat, currentRound) {
  for (const { actor } of uniqueCombatActors(combat)) {
    try {
      const state = vadeState(actor, combat);
      if (!state || state.status !== "pending" || Number(state.lastRound ?? currentRound) >= currentRound) continue;
      if (vadeInitiatingPlayer(state)) continue;
...
export function add2eRegisterRoundEngineHooks() {
  ...
  Hooks.on("updateCombat", (combat, changed, options, userId) => {
    ...
    if (!game.user?.isGM && hasRound) {
      const currentRound = roundNumber(combat, changed?.round);
      continueVadeRetroOnInitiatingPlayer(combat, currentRound)
        .catch(err => error("[VADE_RETRO_PLAYER_ROUND_ERROR]", { err, combat: combat?.id, round: currentRound, userId }));
    }
  ...
  game.add2e.runPlayerVadeRetroContinuation = runPlayerVadeRetroContinuation;
  ...
  globalThis.add2eRunPlayerVadeRetroContinuation = runPlayerVadeRetroContinuation;
