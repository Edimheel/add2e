...
// Version : 2026-07-06-round-engine-vade-retro-flee-v2
// ============================================================================
...
export const ADD2E_ROUND_ENGINE_VERSION = "2026-07-06-round-engine-vade-retro-flee-v2";
...
function actorHp(actor) {
  const sys = actor?.system ?? {};
  for (const value of [sys.pdv, sys.pv?.value, sys.hp?.value, sys.hp, sys.points_de_coup]) {
...
function actorIsDefeated(actor) {
  ...
  return Array.from(actor?.effects ?? []).some(effect => {
    ...
    return toArray(effect?.statuses ?? effect?.getFlag?.("core", "statusId") ?? [])
      .map(norm)
      .some(status => ["dead", "mort", "defeated", "vaincu"].includes(status));
  });
}
...
function combatScene(combat) {
  return combat?.scene ?? game.scenes?.get?.(combat?.scene?.id) ?? game.scenes?.get?.(combat?.sceneId) ?? canvas?.scene ?? null;
}
...
function actorToken(actor, combat) {
...
function actorMovement(actor, scene) {
  ...
  for (const value of [
    ...
    system.mouvement?.max,
    system.mouvement?.base,
    system.mouvement?.modes?.marche?.value,
    system.movement_base,
    system.movement_max,
    system.movement_modes?.marche?.value
  ]) {
...
async function processForcedFlee(actor, combatant, currentRound, combat, { perRound = false } = {}) {
  ...
  if (!scene || !targetToken || !sourceActor || !target || !source) {
    return { moved: false, reason: "missing-token-or-source" };
  }
  ...
  await targetToken.update({ x, y }, {
    add2eRoundEngine: true,
    add2eRoundEngineReason: "forced-flee",
    add2eForcedFlee: true,
...
function vadeGroupTokens(combat, group) {
  const seen = new Set();
  return (Array.isArray(group?.ids) ? group.ids : [])
    .map(id => tokenById(combat, id))
    .filter(token => token?.actor && !actorIsDefeated(token.actor) && !seen.has(token.id) && seen.add(token.id));
}

function vadeQueue(combat, pending) {
  return (Array.isArray(pending) ? pending : [])
    .map(group => ({ ...group, ids: vadeGroupTokens(combat, group).map(token => token.id) }))
...
function vadeDuration(rounds) {
  ...
  return {
    duration: time?.durationData?.(rounds) ?? {
      rounds,
    ...
    flags: time?.flags?.({ source: "vade-retro.js", rounds, unit: "round", endMessage }) ?? {
...
function vadeIsEvil(actor) {
  const alignment = norm(actor?.system?.alignement ?? actor?.system?.alignment ?? "");
  return alignment.includes("mauvais") || alignment.includes("evil");
}
...
async function continueVadeRetro(actor, combat, currentRound) {
  ...
  if (!entry || !targets.length) {
    await saveVadeState(actor, combat, {
      ...state,
      status: targets.length ? "complete" : "pending",
      pending: targets.length ? [] : queue.slice(1),
  ...
  await ChatMessage.create({
    ...
    content: `<div class="add2e-chat-card" style="border:1px solid #c79b38;border-radius:8px;padding:7px 9px;background:#fffaf0;"><b>Vade-rétro continue</b> — round ${currentRound}.<br>Prochaine ligne : <b>${esc(group.label ?? VADE_LABELS[group.category] ?? group.category)}</b>.</div>`
  });
  ...
  if (!automatic && Number(d20.total) < Number(entry)) {
    await saveVadeState(actor, combat, {
      ...
      attempted: { category: group.category, entry, roll: d20.total, result: "Échec", targets: targets.map(token => token.id) }
    });
    await ChatMessage.create({
      speaker: ChatMessage.getSpeaker({ actor }),
      content: `<div class="add2e-chat-card"><b>Vade-rétro — échec</b><br>${esc(group.label ?? VADE_LABELS[group.category] ?? group.category)} : d20 <b>${d20.total}</b>, requis <b>${esc(entry)}</b>. La poursuite cesse.</div>`
    });
    return true;
  }
  ...
  for (const token of affected) {
    ...
    if (destroy) {
      await target.update({ [hpUpdatePath(target)]: 0 }, {
        add2eRoundEngine: true,
        add2eRoundEngineReason: "vade-retro-destruction",
        add2eCombatId: combat.id,
        add2eCombatRound: currentRound
      });
  ...
  await saveVadeState(actor, combat, {
    ...
    lastRound: currentRound,
    updatedAt: Date.now(),
    attempted: { category: group.category, entry, countFormula, count, result: outcome, targets: affected.map(token => token.id) }
  });

  const rollText = automatic ? "résultat automatique" : `d20 ${d20.total} / ${entry}`;
  await ChatMessage.create({
    speaker: ChatMessage.getSpeaker({ actor }),
    content: `<div class="add2e-chat-card" style="border:1px solid #c79b38;border-radius:8px;padding:7px 9px;background:#fffaf0;"><b>Vade-rétro — poursuite</b><br><b>${esc(group.label ?? VADE_LABELS[group.category] ?? group.category)}</b> : ${esc(rollText)} — <b>${esc(outcome)}</b>.<br><b>Nombre affecté :</b> ${esc(countFormula)} = ${count}.${durationLabel ? `<br><b>Durée :</b> ${esc(durationLabel)}.` : ""}<hr>${rows.map(row => `<div><b>${esc(row.name)}</b> — ${esc(row.result)}</div>`).join("")}<div style="margin-top:5px;font-size:.88em;">${status === "pending" ? "Vade-rétro continuera automatiquement au round suivant." : "La séquence de Vade-rétro est terminée."}</div></div>`
  });
...
async function processCombat(combat, { source = "unknown", perRound = false } = {}) {
  ...
  const vadeRetro = perRound ? await continuePendingVadeRetro(combat, currentRound) : null;
  const result = { processed: true, version: ADD2E_ROUND_ENGINE_VERSION, timeEngineVersion: ADD2E_TIME_ENGINE_VERSION, combat: combat.id, round: currentRound, source, perRound, actorCount: actors.length, actors, vadeRetro };
