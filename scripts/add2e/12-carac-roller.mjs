...
// ============================================================
const ADD2E_CARAC_ROLLER_VERSION = "2026-07-05-carac-roller-tiles-and-count-v7";
...
class Add2eCaracRoller {
  ...
  _buildContent() {
    return `
      <style>
        ...
        .add2e-carac-popup .add2e-class-suggestion { min-height:82px!important; }
        .add2e-carac-popup .add2e-class-suggestion .class-name { display:block!important; width:100%!important; min-height:2.1em!important; overflow:visible!important; text-overflow:clip!important; white-space:normal!important; overflow-wrap:anywhere!important; text-align:center!important; font-weight:900!important; line-height:1.05!important; color:inherit!important; }
        ...
        .add2e-carac-popup .reroll { border:1px solid #775122; background:linear-gradient(180deg,#f6dfad,#d19b4c); color:#2d1c0b; }
        .add2e-carac-popup .validate { border:1px solid #6e1414; background:linear-gradient(180deg,#a7372d,#6e1714); color:#fff1d5; }
        .add2e-carac-popup .cancel { border:1px solid #6a5640; background:linear-gradient(180deg,#7b6c5c,#4f463b); color:#fff1d5; }
      </style>
      <div class="add2e-carac-popup" data-add2e-carac-roller="${this._uid}" style="box-sizing:border-box;width:100%;min-width:100%;max-width:100%;padding:10px;color:#2a1b0d;background:linear-gradient(180deg,#efe0bc 0%,#d8bd82 100%);border:2px solid #5a3418;border-radius:8px;box-shadow:inset 0 0 0 1px rgba(255,255,255,.35);">
        ...
        <div id="classes-suggestions" style="margin:0 0 9px 0;padding:8px 10px;border:1px solid #8a6330;border-radius:8px;background:rgba(43,28,13,.10);max-height:360px;overflow:auto;"></div>
  ...
  _bindDialogEvents() {
    ...
    this._dlgRoot.querySelector(".apply-caracs-btn")?.addEventListener("click", event => { event.preventDefault(); this.apply(); });
  ...
  _classSuggestionPlan(cls) {
    ...
    for (const requirement of requis) {
      const poolIndex = pool.findIndex(entry => entry.value + add2eCaracRaceBonus(this.actor, requirement.carac) >= requirement.min);
  ...
  async classesSynthese() {
    ...
    for (const cls of classes) {
      ...
      cards += `<button type="button" class="add2e-class-suggestion" data-plan-key="${key}" title="Auto-affecter les prérequis" style="${this._classTagStyle(cls.name)}"><b class="class-name">${add2eCaracEscapeHtml(cls.name)}</b>${detail}</button>`;
    }

    const heading = `<div style="margin:0 0 6px;font-size:.82rem;color:#5b1e16;font-weight:900;">Classes possibles : ${count}<span style="font-size:.68rem;font-weight:700;color:#6a4a28;"> / ${classes.length} chargée${classes.length > 1 ? "s" : ""}</span></div>`;
    const body = count ? cards : "<em>Aucune classe ne correspond à ce tirage.</em>";
    return `${heading}<div class="add2e-class-tags" style="display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:7px;align-items:stretch;width:100%;">${body}</div>`;
  }
