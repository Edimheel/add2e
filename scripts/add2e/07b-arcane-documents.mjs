...
function actorLists(actor, allowedLists = SCROLL_LISTS) {
...
function actorScrollLists(actor) {
...
async function detachPersonalSpellbooks(actor, { reason = "loot-death" } = {}) {
...
async function restorePersonalBookSync(actor, { reason = "actor-restored" } = {}) {
...
function arcaneChatCard({ actor = null, title = "Document arcanique", source = "", result = "", details = [], status = "", icon = "fa-book", image = "" } = {}) {
...
async function createArcaneChatMessage({ actor = null, ...card } = {}) {
...
async function handleActorMortality(actor, options = {}) {
  ...
  if (!actorDead(actor) && actor.getFlag?.("add2e", "arcaneBooksDetachedForLoot") === true) await restorePersonalBookSync(actor, { reason: "hostile-restored" });
}
