...
// Version : 2026-06-15-racial-bonus-drop-cap-v3
// ============================================================

const ADD2E_RACE_CLASS_DROP_VERSION = "2026-06-15-racial-bonus-drop-cap-v3";
...
const CARAC_SHORT = { force: "FOR", dexterite: "DEX", constitution: "CON", intelligence: "INT", sagesse: "SAG", charisme: "CHA" };
...
function add2eDropNumber(value, fallback = 0) {
  const n = Number(value);
  return Number.isFinite(n) ? n : fallback;
}

function add2eActorBaseCaracForRaceDrop(actor, carac) {
  return add2eDropNumber(actor?.system?.[`${carac}_base`] ?? actor?.system?.[carac], 10);
}

function add2eClampRaceBonusForBase(base, bonus) {
...
function add2eClampRaceBonusesForExistingBases(actor, rawBonuses = {}, context = {}) {
...
function add2eRaceAdjustedCaracTotal(actor, carac, rawBonuses = {}) {
  const base = add2eActorBaseCaracForRaceDrop(actor, carac);
  ...
  SheetClass.prototype._onDrop = async function add2eDropCompatPopupWrapped(event) {
    ...
    try {
      ...
      if (raw?.type === "Item") {
        ...
        if (!itemData && raw.uuid) { const doc = await fromUuid(raw.uuid); if (doc instanceof Item) itemData = doc.toObject(); }
        if (!itemData && raw.pack && raw.id) { const pack = game.packs.get(raw.pack); const ent = pack && await pack.getDocument(raw.id); if (ent instanceof Item) itemData = ent.toObject(); }
      }
    } catch (e) { console.warn("[ADD2E][DROP][RACE_CLASSE] Impossible de lire les données de drop.", e); }
    if (itemData && ["classe", "race"].includes(itemData.type)) {
      ...
      if (!resolved.ok || resolved.handled) { event.preventDefault(); event.stopPropagation(); return false; }
    }
