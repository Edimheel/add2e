// ADD2E — Multiclassage : fenêtres via l’API commune ADD2E.
...
import {
  monoClassOptionsForDroppedClass,
...
function add2eMulticlassDialogWait() {
...
export async function dialogAlert(title, content, { classes = ["add2e-multiclass-alert"], okLabel = "OK" } = {}) {
  return add2eMulticlassDialogWait()({
    add2eTheme: "parchment",
    add2ePrimaryAction: "ok",
    add2eClasses: classes,
    window: { title },
    content,
    buttons: [{
      action: "ok",
...
export async function dialogWait({ title, content, buttons, classes = [] }) {
  const primaryAction = buttons?.find?.(button => button?.default === true)?.action ?? "";
  return add2eMulticlassDialogWait()({
    add2eTheme: "parchment",
    add2ePrimaryAction: primaryAction,
    add2eClasses: classes,
    window: { title },
    content,
    buttons,
...
export function replacementOptionsForDroppedClass(actor, droppedClassData, currentRaceOrCompatibleAlternatives) {
  ...
  for (const replacedClass of existing) {
    ...
    const raceChoices = currentRaceOrCompatibleAlternatives(
      actor,
      race => raceCompatibleForReplacement(actor, droppedClassData, replaceSlug, race)
    );
    for (const raceData of raceChoices) {
      ...
      out.push({
        ...
        raceData,
        needsRaceChange: raceKey !== norm(itemLabel(systemRace(actor), "Race")),
...
function add2eStatValue(actor, carac) {
  ...
  if (!engine || typeof engine.resolveAbility !== "function") {
    throw new Error("Le résolveur canonique ADD2E des caractéristiques est indisponible pour le multiclassage.");
  }
  const resolution = engine.resolveAbility(actor, carac, {
    type: "multiclass-prerequisite-display",
    source: "multiclass-dialogs",
    consumer: "multiclass-ui"
  });
...
function add2eStatFailureBody(actor, droppedClassData, _currentRaceOrCompatibleAlternatives, _isAlreadyMulticlass) {
  ...
  if (!rows.length) {
    return `<div style="padding:12px;border:1px solid #8f2a20;border-radius:10px;background:#f0c6b4;color:#6b1b12;font-weight:900;">Aucune option valide pour cette classe.</div>`;
  }
...
export async function showClassDropChoiceDialog(actor, droppedClassData, currentRaceOrCompatibleAlternatives) {
  ...
  const addOptions = raceCandidatesForClass(actor, droppedClassData)
    .map(raceData => ({ classData: droppedClassData, raceData }));
  const replacementOptions = isAlreadyMulticlass
    ? replacementOptionsForDroppedClass(actor, droppedClassData, currentRaceOrCompatibleAlternatives)
    : [];
  ...
  const body = sections || add2eStatFailureBody(
    actor,
    droppedClassData,
  ...
  const readChoice = (_event, button, dialog) => {
    const form = button?.form
      ...
      ?? document.querySelector?.(".add2e-multiclass-choice")?.closest?.("form");
    const raw = form?.elements?.add2eChoice?.value
      ?? dialog?.element?.querySelector?.('input[name="add2eChoice"]:checked')?.value
      ?? "cancel";
    if (raw.startsWith("monoclass:")) {
      return { action: "monoclass", option: monoOptions[Number(raw.split(":")[1]) || 0] ?? null };
    }
    if (raw.startsWith("multiclass:")) {
      return { action: "multiclass", option: addOptions[Number(raw.split(":")[1]) || 0] ?? null };
    }
    if (raw.startsWith("replace:")) {
      return { action: "replace-class", option: replacementOptions[Number(raw.split(":")[1]) || 0] ?? null };
    }
    return { action: "cancel" };
  };

  return dialogWait({
    title: "ADD2E — Classe ou multiclassage",
    content,
...
export async function notifyLevelCap(actor, slug, requestedLevel, appliedLevel, payload) {
  ...
  const raceName = itemLabel(systemRace(actor), "Race");
  const content = `
    <div class="add2e-level-cap-dialog" style="display:grid;gap:10px;min-width:420px;max-width:560px;color:#2b1c0d;">
      <div style="border:1px solid #6f4515;border-radius:12px;background:linear-gradient(180deg,#4b2b0f,#1f1207);padding:10px 12px;color:#ffe39d;box-shadow:0 2px 8px rgba(0,0,0,.25);">
        <h2 style="margin:0;border:0;color:#ffe39d;font-size:1.08rem;text-transform:uppercase;letter-spacing:.04em;">Niveau maximum atteint</h2>
        <p style="margin:5px 0 0;color:#fff3cf;font-weight:700;">La limite raciale empêche cette progression.</p>
      </div>
      <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;">
        <div style="border:1px solid #c59a3d;border-radius:10px;background:#fffaf0;padding:8px 10px;"><span style="display:block;color:#6b470f;font-size:.72rem;font-weight:900;text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px;">Classe</span><b>${esc(className)}</b></div>
        <div style="border:1px solid #c59a3d;border-radius:10px;background:#fffaf0;padding:8px 10px;"><span style="display:block;color:#6b470f;font-size:.72rem;font-weight:900;text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px;">Race</span><b>${esc(raceName)}</b></div>
        <div style="border:1px solid #c59a3d;border-radius:10px;background:#fffaf0;padding:8px 10px;"><span style="display:block;color:#6b470f;font-size:.72rem;font-weight:900;text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px;">Niveau demandé</span><b>${esc(requestedLevel)}</b></div>
        <div style="border:1px solid #c59a3d;border-radius:10px;background:#fffaf0;padding:8px 10px;"><span style="display:block;color:#6b470f;font-size:.72rem;font-weight:900;text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px;">Niveau appliqué</span><b>${esc(appliedLevel)}</b></div>
      </div>
      <div style="border:1px solid #9d2d25;border-radius:10px;background:#ffe5df;color:#7b1f18;padding:9px 11px;font-weight:800;">${esc(className)} ne peut pas dépasser le niveau ${esc(maxLevel)} pour cette race.</div>
    </div>`;
  return dialogAlert("ADD2E — Niveau maximum atteint", content, {
