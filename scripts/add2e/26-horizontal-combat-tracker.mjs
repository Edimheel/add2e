...
export const ADD2E_HORIZONTAL_TRACKER_VERSION = "2026-07-30-horizontal-combat-tracker-token-filter-v10";
...
const TOKEN_MOVEMENT_KEYS = new Set(["x", "y", "elevation", "rotation"]);
...
function tokenUpdateAffectsTracker(changes = {}) {
  const keys = Object.keys(changes ?? {});
  if (!keys.length) return false;
  return keys.some(key => !TOKEN_MOVEMENT_KEYS.has(String(key).split(".")[0]));
}
...
function actionKindPresentation(action) {
  if (action?.kind === "weapon") return { label: "Arme", icon: "fas fa-sword", segmentLabel: "rapidité", short: "R" };
  if (action?.kind === "spell") return { label: "Sort", icon: "fas fa-wand-magic-sparkles", segmentLabel: "incantation", short: "T" };
  if (action?.kind === "item") return { label: "Pouvoir", icon: "fas fa-wand-sparkles", segmentLabel: "activation", short: "S" };
  return { label: "Action", icon: "fas fa-bolt", segmentLabel: "segment", short: "S" };
}
...
function initiativePresentation(combatant, combat) {
  ...
  const actionLabel = action
    ? `${action.label}${Number.isFinite(segment) ? ` · ${kind.short}${segment}` : ""}`
    : "";
  const actionTitle = action
    ? `${kind.label} choisie : ${action.label}${Number.isFinite(segment) ? ` — ${kind.segmentLabel} ${segment}` : ""}`
...
function visibleSidebarRect() {
  for (const selector of ["#sidebar", "#ui-right-column", "#interface #sidebar", ".sidebar-popout"]) {
...
function ensureStyles() {
  ...
  style.id = STYLE_ID;
  style.textContent = `
    #add2e-horizontal-combat-tracker{position:fixed!important;top:10px!important;height:auto!important;z-index:90!important;background:transparent!important;border:0!important;box-shadow:none!important;pointer-events:none!important}
    #add2e-horizontal-combat-tracker .window-header{display:none!important}
    #add2e-horizontal-combat-tracker .window-content{padding:0!important;overflow:visible!important;background:transparent!important;pointer-events:none!important}
    .add2e-horizontal-tracker-shell{display:flex;align-items:center;gap:10px;width:100%;padding:2px 0;background:transparent;pointer-events:none;color:#f1e7c6}
    .add2e-horizontal-controls{display:flex;flex:0 0 auto;align-items:center;gap:7px;pointer-events:auto}
    .add2e-horizontal-round{min-width:98px;padding:8px 10px;border:1px solid rgba(205,170,82,.8);border-radius:9px;background:rgba(54,43,23,.82);box-shadow:0 3px 10px rgba(0,0,0,.35);text-align:center;font-size:1rem;font-weight:900;color:#f0cf72;backdrop-filter:blur(3px)}
    .add2e-horizontal-round small{display:block;margin-bottom:2px;font-size:.7rem;color:#d8cfb5}
    .add2e-horizontal-control{display:inline-grid;place-items:center;width:42px;height:42px;padding:0;border:1px solid #9d8038;border-radius:9px;background:rgba(67,52,25,.88);box-shadow:0 3px 10px rgba(0,0,0,.35);color:#f8e9b1;font-size:1rem;cursor:pointer;backdrop-filter:blur(3px)}
    ...
    .add2e-horizontal-list{display:flex;flex:1 1 auto;align-items:flex-end;gap:10px;min-width:0;overflow-x:auto;overflow-y:visible;padding:10px 8px 12px;pointer-events:none;scrollbar-width:thin;scrollbar-color:#8b7134 transparent}
    .add2e-horizontal-card{position:relative;flex:0 0 110px;width:110px;height:140px;overflow:hidden;padding:0;border:2px solid rgba(111,104,88,.95);border-radius:10px;background:transparent;box-shadow:0 4px 11px rgba(0,0,0,.52);transition:transform .14s ease,filter .14s ease,border-color .14s ease;pointer-events:auto}
    .add2e-horizontal-card.active{transform:translateY(-5px) scale(1.07);border:4px solid #d94335;box-shadow:0 0 0 2px rgba(255,210,76,.72),0 7px 16px rgba(0,0,0,.58);z-index:4}
    .add2e-horizontal-card.inactive{filter:grayscale(.9) brightness(.55)}
    .add2e-horizontal-card.hidden-combatant{filter:saturate(.3)}
    .add2e-horizontal-card img{position:absolute;inset:0;display:block;width:100%;height:100%;margin:0;border:0;border-radius:0;object-fit:cover;object-position:center;cursor:pointer}
    .add2e-horizontal-card::after{content:"";position:absolute;inset:auto 0 0;height:58%;pointer-events:none;background:linear-gradient(transparent,rgba(0,0,0,.86))}
    .add2e-horizontal-init{position:absolute;top:5px;right:5px;display:grid;place-items:center;min-width:32px;height:32px;padding:0 7px;border:2px solid #e0c45e;border-radius:999px;background:rgba(24,26,30,.9);color:#fff1b2;font-size:.92rem;font-weight:900;z-index:6;cursor:pointer}
    .add2e-horizontal-name{position:absolute;left:5px;right:5px;bottom:5px;z-index:5;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:center;font-size:.78rem;font-weight:900;color:#fff8df;text-shadow:0 1px 3px #000,0 0 4px #000}
    .add2e-horizontal-state{position:absolute;left:6px;top:6px;display:grid;place-items:center;width:28px;height:28px;border:1px solid #efaaaa;border-radius:999px;background:rgba(123,31,31,.94);color:#fff;z-index:6}
    .add2e-horizontal-meta{position:absolute;left:5px;right:5px;bottom:25px;z-index:5;display:grid;gap:2px;pointer-events:none}
    .add2e-horizontal-card.has-attacks .add2e-horizontal-meta{bottom:50px}
    .add2e-horizontal-meta-line{display:flex;align-items:center;justify-content:center;gap:4px;min-width:0;padding:3px 4px;border:1px solid rgba(222,195,111,.6);border-radius:4px;background:rgba(24,26,30,.84);color:#fff4c9;font-size:.62rem;font-weight:800;line-height:1.05}
    .add2e-horizontal-meta-line span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    ...
    .add2e-horizontal-attacks{position:absolute;left:5px;right:5px;bottom:25px;z-index:5;padding:3px 4px;border-radius:4px;background:rgba(0,0,0,.72);font-size:.64rem;line-height:1.08;text-align:center;color:#fff2b4}
    .add2e-horizontal-menu{position:fixed;z-index:300;display:grid;min-width:235px;max-width:310px;padding:6px;border:1px solid #947a3d;border-radius:8px;background:#202328;box-shadow:0 8px 24px rgba(0,0,0,.72)}
    .add2e-horizontal-menu button{display:flex;align-items:center;gap:8px;padding:7px 9px;border:0;background:transparent;color:#eee6cf;text-align:left;cursor:pointer}
    .add2e-horizontal-menu button:hover{background:#4b4029}
    .add2e-horizontal-menu button.danger{color:#ffaaa2}
    .add2e-initiative-situation-form{display:grid;gap:12px;padding:4px}
    .add2e-initiative-situation-form .form-group{display:grid;grid-template-columns:minmax(155px,1fr) minmax(90px,.65fr);align-items:center;gap:10px;margin:0}
    .add2e-initiative-situation-form input[type="number"]{width:100%}
    .add2e-initiative-situation-form .checkbox{display:flex;align-items:center;justify-content:space-between;gap:12px}
    .add2e-initiative-situation-summary{padding:8px 10px;border:1px solid rgba(143,119,57,.65);border-radius:6px;background:rgba(143,119,57,.1);line-height:1.35}
    @media(max-width:800px){#add2e-horizontal-combat-tracker{top:4px!important}.add2e-horizontal-tracker-shell{gap:6px}.add2e-horizontal-controls{gap:4px}.add2e-horizontal-round{min-width:76px;padding:6px;font-size:.84rem}.add2e-horizontal-control{width:36px;height:36px}.add2e-horizontal-control.start{min-width:92px;padding:0 8px}.add2e-horizontal-card{flex-basis:96px;width:96px;height:124px}.add2e-horizontal-meta-line{font-size:.57rem}}
  `;
...
async function editInitiativeSituation(combatant, combat) {
  ...
  const presentation = actionKindPresentation(action);
  const actionText = action
    ? `${esc(action.label)}${Number.isFinite(Number(action.segment)) ? ` — ${presentation.segmentLabel} ${esc(action.segment)}` : ""}`
    : "Aucune action choisie";
  ...
  const result = await DialogV2.wait({
    ...
    content: `
      <form class="add2e-initiative-situation-form">
        ...
        <div class="form-group"><label for="add2e-initiative-modifier">Modificateur de situation</label><input id="add2e-initiative-modifier" type="number" name="modifier" step="1" value="${esc(Number(current?.modifier) || 0)}"></div>
        <div class="form-group"><label for="add2e-surprise-segments">Segments de surprise</label><input id="add2e-surprise-segments" type="number" name="surpriseSegments" min="0" step="1" value="${esc(Math.max(0, integer(current?.surpriseSegments)))}"></div>
        <label class="checkbox" for="add2e-apply-dexterity-reaction"><span>Appliquer la réaction de Dextérité</span><input id="add2e-apply-dexterity-reaction" type="checkbox" name="applyDexterityReaction" ${current?.applyDexterityReaction !== false ? "checked" : ""}></label>
      </form>`,
    buttons: [
      {
        action: "save",
  ...
  await add2eSetInitiativeSituation(combatant, { ...result, source: "horizontal-tracker" }, combat);
  return true;
}

async function clearInitiativeAction(combatant, combat) {
  const action = add2eGetDeclaredInitiativeAction(combatant, combat);
  if (!action) return ui.notifications?.info?.("Aucune action d’initiative n’est choisie.");
  const confirmed = await confirmAction({
    title: "Effacer l’action choisie",
...
async function rollCombatants(combat, ids) {
  ...
  return add2eRollInitiative(combat, allowedIds, {
    updateTurn: false,
    messageMode: trackerMessageMode()
  });
}
...
async function advanceRound(combat) {
  ...
  if (typeof combat.nextRound !== "function") throw new Error("L’API publique Combat.nextRound est indisponible.");
...
class Add2eHorizontalCombatTracker extends ApplicationV2 {
  ...
  async _renderHTML(context) {
    ...
    const cards = context.combatants.map(combatant => {
      const title = [combatant.name, combatant.skipReason, combatant.hasAction ? combatant.actionTitle : "", combatant.hasSituation ? combatant.situationTitle : ""].filter(Boolean).join(" — ");
      ...
      return `
        <article class="add2e-horizontal-card${combatant.active ? " active" : ""}${combatant.inactive ? " inactive" : ""}${combatant.hidden ? " hidden-combatant" : ""}${combatant.attackStatus ? " has-attacks" : ""}" data-combatant-id="${esc(combatant.id)}" title="${esc(title)}">
          <img src="${esc(combatant.image)}" alt="${esc(combatant.name)}" data-action="focus-token">
          <button type="button" class="add2e-horizontal-init" data-action="roll-one" ${combatant.canManage ? "" : "disabled"} title="${combatant.hasInitiative ? "Relancer l’initiative" : "Lancer l’initiative au d6"}">${combatant.hasInitiative ? esc(combatant.initiative) : '<i class="fas fa-dice-d6"></i>'}</button>
  ...
  async _onAction(event) {
    ...
    if (action === "roll-missing") {
      const ids = Array.from(combat.combatants ?? []).filter(entry => entry.initiative === null || entry.initiative === undefined).map(entry => entry.id);
  ...
  _openContextMenu(event) {
    ...
    menu.innerHTML = `
      ...
      ${action ? '<button data-menu-action="clear-action"><i class="fas fa-eraser"></i> Effacer l’action choisie</button>' : ""}
...
const REFRESH_HOOKS = [
  ...
  "updateActor", "canvasReady", "collapseSidebar",
...
Hooks.on("updateToken", (_token, changes) => {
  if (tokenUpdateAffectsTracker(changes)) refreshTracker();
});
