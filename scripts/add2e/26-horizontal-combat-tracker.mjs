...
export const ADD2E_HORIZONTAL_TRACKER_VERSION = "2026-07-14-horizontal-combat-tracker-v1";
...
const SKIP_STATUS_IDS = new Set([
  ...
  "paralyzed", "paralysed", "paralyse", "paralysé", "paralysé",
  "petrified", "petrifie", "pétrifié", "pétrifié",
  "stunned", "etourdi", "étourdi", "étourdi",
  "asleep", "sleeping", "endormi", "neutralized", "neutralise", "neutralisé"
]);
...
function trackerCombat() {
  ...
  return combat?.started && combat?.combatants?.size ? combat : null;
}
...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .add2e-horizontal-round{min-width:72px;text-align:center;font-weight:900;color:#f0cf72}.add2e-horizontal-round small{display:block;font-size:.67rem;color:#bbb29b}
...
async function rollCombatants(combat, ids) {
  ...
  await combat.rollInitiative(allowedIds, {
    updateTurn: false,
...
class Add2eHorizontalCombatTracker extends Add2eApplicationV2 {
  ...
  async _prepareContext() {
    ...
    const combatants = sortedCombatants(combat)
      ...
      .map(combatant => {
        ...
        const attackStatus = add2eMultipleAttackHudStatus(combatant.actor, combat);
        return {
          id: combatant.id,
  ...
  async _renderHTML(context) {
    ...
    root.innerHTML = `
      <div class="add2e-horizontal-tracker-shell">
        <div class="add2e-horizontal-round"><small>COMBAT</small>Round ${context.round}</div>
        <button type="button" class="add2e-horizontal-control previous" data-action="previous-turn" title="Tour précédent"><i class="fas fa-chevron-left"></i></button>
        <div class="add2e-horizontal-list">${cards}</div>
        <button type="button" class="add2e-horizontal-control" data-action="next-turn" title="Tour suivant"><i class="fas fa-chevron-right"></i></button>
        <div class="add2e-horizontal-actions">
          ...
          ${context.isGM ? '<button type="button" class="add2e-horizontal-control" data-action="end-combat" title="Terminer le combat"><i class="fas fa-flag-checkered"></i></button>' : ""}
        </div>
      </div>`;
    return root;
  }
  ...
  async _onAction(event) {
    ...
    if (action === "next-turn") return advanceAndSkip(combat, 1);
    if (action === "previous-turn") return advanceAndSkip(combat, -1);
    ...
    if (action === "end-combat" && game.user?.isGM) {
  ...
  _openContextMenu(event) {
    ...
    menu.dataset.combatantId = combatant.id;
