...
function add2eMagicItemIsPotion(item) {
  ...
  const markers = [
    ...
    sys.forme,
    sys.kind,
    ...add2eObjectMagicToArray(sys.tags),
    ...add2eObjectMagicToArray(sys.effectTags),
...
export async function add2eObjectPowerSetCharges(itemSource, power, idx, value) {
  ...
  if (globalMax > 0) {
    ...
    if (sys?.charges && typeof sys.charges === "object") await itemSource.update({ "system.charges.value": next });
    return;
  }
...
export function add2eBuildVirtualObjectPowerSort(actor, itemSource, power, idx) {
  ...
  const fakeData = {
    ...
    flags: { add2e: { memorizedCount: current, originalOnUse: onUse, sourceType: "objet_magique", sourceItemId: itemSource.id, sourceItemName: itemSource.name, powerIndex: idx } }
  };
...
export function add2eMagicItemEquippedOrUsable(item) {
  const sys = item?.system ?? {};
  if (add2eMagicItemIsPotion(item)) return true;
...
export function add2eMagicObjectRawPowers(item) {
  ...
  const sys = item?.system ?? {};
  return sys.pouvoirs ?? sys.powers ?? sys.pouvoirsMagiques ?? sys.magicalPowers ?? sys.sorts ?? sys.spells ?? [];
}
...
export function add2eMagicObjectActivePowerEntries(item) {
  return add2eMagicObjectPowerArray(item).map((power, index) => ({ power, index })).filter(entry => add2eObjectPowerOnUsePath(entry.power));
}
...
export function add2eMagicObjectChargeInfo(item, powers = null) {
  ...
  const maxCandidates = [sys.max_charges, sys.maxCharges, sys.charges_max, sys.chargesMax, sys.max, sys.charges?.max, sys.charges?.maximum, ...list.map(p => p.max ?? p.maxCharges ?? p.chargesMax ?? p.charges_max)];
  let max = add2eMagicReadNumber(...maxCandidates);
  if (!Number.isFinite(max) || max < 0) max = 0;
  const currentCandidates = [item?.getFlag?.("add2e", "global_charges"), item?.getFlag?.("add2e", "charges"), sys.charges, sys.current_charges, sys.currentCharges, sys.charges_actuelles, sys.chargesRestantes, sys.remainingCharges, sys.charges?.value, sys.charges?.current, sys.charges?.actuel, sys.charges?.remaining];
...
export function add2eMagicLooksMagical(item) {
  ...
  return tags.some(t => t.includes("magique") || t.includes("magic")) || name.includes("magique") || sys.magique === true || sys.magic === true;
}
...
export function add2eUiCollectObjectMagicGroups(actor) {
  ...
  for (const itemSource of itemsSources) {
    ...
    for (const { power: p, index: idx } of powerEntries) {
      ...
      const charges = isGlobal ? chargeInfo.current : (itemSource.getFlag?.("add2e", `charges_${idx}`) ?? p.charges ?? itemSource.system?.charges ?? max);
      powers.push({ id: add2eMagicPowerGeneratedId(itemSource, idx), name: String(p.name || p.nom || itemSource.name || "Pouvoir").trim() || "Pouvoir", img: p.img || itemSource.img || "icons/svg/aura.svg", sourceItemId: itemSource.id, sourceName: itemSource.name || "Objet magique", sourceImg: itemSource.img || "icons/svg/item-bag.svg", niveau: Number(p.niveau ?? p.level ?? 1) || 1, description: p.description || p.desc || "", charges: Number(charges) || 0, max, cost: Number(p.cout ?? p.cost ?? 0) || 0 });
    }
    groups.push({ itemId: itemSource.id, itemName: itemSource.name || "Objet magique", itemImg: itemSource.img || "icons/svg/item-bag.svg", charges: chargeInfo.current, max: chargeInfo.max, chargeLabel: chargeInfo.label, powers: powers.sort((a, b) => String(a.name).localeCompare(String(b.name), "fr")) });
  }
...
export function add2eUiBuildObjectMagicSection(actor) {
  ...
  const content = groups.length ? groups.map(group => {
    const rows = group.powers.length ? group.powers.map(power => `
      ...
      </tr>`).join("") : `<tr><td colspan="3" class="a2e-muted" style="text-align:center;padding:0.6em;">Aucun pouvoir détaillé.</td></tr>`;
    return `<div class="add2e-object-magic-group" data-item-id="${add2eObjectMagicEscapeHtml(group.itemId)}" style="border:1px solid #d9bf73;border-radius:9px;margin-bottom:8px;background:#fffdf6;overflow:hidden;"><div class="add2e-object-magic-header" style="display:flex;align-items:center;gap:8px;padding:7px 9px;background:#ead99d;border-bottom:1px solid #dac276;color:#3d2b0a;font-weight:900;"><img src="${add2eObjectMagicEscapeHtml(group.itemImg)}" alt="" style="width:28px;height:28px;border:1px solid #6f4b12;border-radius:6px;object-fit:cover;"><span style="flex:1;">${add2eObjectMagicEscapeHtml(group.itemName)}</span><span title="Charges restantes / charges maximum" style="padding:2px 8px;border:1px solid #9f7a24;border-radius:999px;background:#fffaf0;white-space:nowrap;">Charges ${add2eObjectMagicEscapeHtml(group.chargeLabel)}</span></div><table class="a2e-table add2e-object-magic-table" style="margin:0;"><thead><tr><th style="width:46px;">Utiliser</th><th>Pouvoir</th><th>Description</th></tr></thead><tbody>${rows}</tbody></table></div>`;
  }).join("") : `<div class="a2e-muted" style="text-align:center;padding:0.8em;border:1px solid #dac276;border-radius:9px;background:#fffdf6;">Aucun objet magique doté d’un pouvoir utilisable.</div>`;
...
export function add2eUiInjectObjectMagicSection(spellContainer, actor) {
  ...
  const firstSpellPanel = Array.from(spellContainer.querySelectorAll(".a2e-panel")).find(p => p.querySelector?.("table.sort-table"));
