...
const ADD2E_MONK_RULES_VERSION = "2026-07-08-passive-abilities-v1";
...
function add2eMonkPercent(value, fallback = 0) {
  const number = add2eMonkNumber(value);
  return Number.isFinite(number) ? Math.max(0, Math.min(100, number)) : fallback;
}
function add2eMonkEspResistance(level, row) {
  const direct = add2eMonkNumber(row?.resistESP, row?.resistance_esp);
  if (Number.isFinite(direct) && direct > 0) return Math.max(0, Math.min(100, direct));
  return level >= 4 ? Math.min(100, 70 + ((level - 4) * 2)) : 0;
}
function add2eMonkCharmResistance(level, row) {
  const direct = add2eMonkNumber(row?.resistCharmSuggestion, row?.resistance_charme_suggestion);
  if (Number.isFinite(direct) && direct > 0) return Math.max(0, Math.min(100, direct));
  return level >= 9 ? Math.min(100, 50 + ((level - 9) * 5)) : 0;
}
function add2eMonkWeaponDamageBonus(level, row) {
  const direct = add2eMonkNumber(row?.monkWeaponDamageBonus, row?.bonusDegatsArme, row?.bonus_degats_arme);
  return Number.isFinite(direct) ? direct : Math.max(0, level / 2);
}
function add2eMonkFeature(key, minLevel, name, description, tags = []) {
  return {
...
function add2eBuildMonkPassiveFeatures(actor, monk, row) {
  ...
  const weaponDamageBonus = add2eMonkWeaponDamageBonus(level, row);
  const resistESP = add2eMonkEspResistance(level, row);
  const resistCharm = add2eMonkCharmResistance(level, row);
  const mentalDefenseInt = add2eMonkNumber(row?.mentalDefenseIntelligence, row?.defenseMentaleIntelligence);
  ...
  const features = [
    ...
    add2eMonkFeature("bonus_degats_martial", 1, "Bonus de dégâts martial", `Avec les armes autorisées autres que Main nue, le moine ajoute +${weaponDamageBonus} aux dégâts.`, ["bonus_degats:moine:demi_niveau"]),
    add2eMonkFeature("ajustements_physiques", 1, "Ajustements physiques particuliers", "La Dextérité ne donne pas d’ajustement à la CA du moine et les bonus de Force habituels au toucher ou aux dégâts sont ignorés.", ["dex_ca:ignore", "force_bonus:toucher:ignore", "force_bonus:degats:ignore"]),
  ...
  if (level >= 4) features.push(add2eMonkFeature("esprit_masque", 4, "Masquer son esprit", `L’ESP n’a qu’une chance réduite d’atteindre le moine. Résistance actuelle : ${resistESP} %.`, ["moine:esprit_masque", `resistance:esp:${resistESP}`]));
  ...
  if (level >= 9) features.push(add2eMonkFeature("resistance_charme_suggestion", 9, "Résistance aux charmes et suggestions", `Les charmes, suggestions, hypnotismes et séductions n’ont que peu de chance d’affecter le moine. Résistance actuelle : ${resistCharm} %.`, ["moine:resistance_charme_suggestion", `resistance:charme_suggestion:${resistCharm}`]));
  if (level >= 10 || mentalDefenseInt === 18) features.push(add2eMonkFeature("defense_mentale", 10, "Défense mentale", "Contre les attaques télépathiques ou les chocs mentaux, le moine est traité comme ayant 18 en Intelligence.", ["moine:defense_mentale", "defense_mentale:intelligence_18"]));
...
async function add2eEnsureMonkPassiveClassFeatures(actor, monk, row) {
  ...
  const generatedNames = new Set(generated.map(feature => add2eMonkNorm(feature.name)));
  const current = add2eMonkFeatureArray(monk.system?.classFeatures);
  const preserved = current.filter(feature => {
    const generatedBy = feature?.flags?.add2e?.generatedBy;
    if (generatedBy === ADD2E_MONK_GENERATED_FEATURE_SOURCE) return false;
    if (feature?.passive === true && generatedNames.has(add2eMonkNorm(feature?.name))) return false;
    return true;
  });
  const next = [...preserved, ...generated];
  if (JSON.stringify(current) === JSON.stringify(next)) return false;
  await monk.update({ "system.classFeatures": next }, { add2eInternal: true, add2eReason: "monk-passive-features-sync" });
  return true;
}
function add2eMonkPassiveSnapshot(monk, row) {
  const level = add2eMonkClassLevel(monk) ?? 1;
  const slowFall = add2eMonkNumber(row?.slowFall, row?.chute_ralentie);
  const weaponDamageBonus = add2eMonkWeaponDamageBonus(level, row);
  const resistESP = add2eMonkEspResistance(level, row);
  const resistCharmSuggestion = add2eMonkCharmResistance(level, row);
  return {
    level,
    title: row?.title ?? row?.titre ?? "",
    armorClass: add2eMonkNumber(row?.monkAC, row?.caMoine, row?.ca_moine),
    movement: add2eMonkNumber(row?.move, row?.movement, row?.mouvement),
    attacksPerRound: String(row?.attacksPerRound ?? row?.attaquesParRound ?? "").trim(),
    unarmedDamage: String(row?.unarmedDamage ?? row?.main_nue ?? row?.damage ?? "").trim(),
    openDoors: add2eMonkNumber(row?.openDoors, row?.ouvrir_portes),
    slowFall,
    weaponDamageBonus,
    noDexArmorClass: true,
    ignoreStrengthHitBonus: true,
    ignoreStrengthDamageBonus: true,
    projectileParry: true,
    thiefSavingThrows: true,
    noDamageOnSuccessfulSave: true,
    halfDamageOnFailedSave: level >= 9,
    animalLanguage: level >= 3,
    plantLanguage: level >= 8,
    espResistance: resistESP,
    charmSuggestionResistance: resistCharmSuggestion,
    immuneDisease: level >= 5 || row?.immuneDisease === true,
    immuneHasteSlow: level >= 5 || row?.immuneHasteSlow === true,
    catalepsyTurnsPerLevel: level >= 6 ? 2 : 0,
    mentalDefenseIntelligence: level >= 10 ? 18 : null,
    immunePoison: level >= 11 || row?.immunePoison === true,
    immuneQuestGeas: level >= 12 || row?.immuneQuestGeas === true,
    quiveringPalm: level >= 13 || row?.quiveringPalm === true
  };
}
...
async function add2eSyncMonkUnarmedWeapon(actor) {
  ...
  const passifs = add2eMonkPassiveSnapshot(monk, row);
  await actor.update({
