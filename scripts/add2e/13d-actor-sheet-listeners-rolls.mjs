...
export const ADD2E_SHEET_ROLL_DELEGATION_VERSION = "2026-07-08-effects-engine-save-rolls-v2";
...
function add2eSheetRollSavingThrowsFromEffectsEngine(actor) {
  const engine = add2eSheetRollEffectsEngine();
  if (!actor || !engine) return null;
  if (typeof engine.getEmbeddedClassItems !== "function") return null;
  if (typeof engine.getClassProgressionEntryForPassiveRule !== "function") return null;

  const classItems = engine.getEmbeddedClassItems(actor);
  for (const classItem of classItems) {
    ...
    const saves = progression?.savingThrows ?? progression?.sauvegardes;
    if (!Array.isArray(saves) || !saves.length) continue;
    return {
      saves,
      progression,
      classItem,
      classLevel,
      source: "effects-engine"
    };
  }
...
function add2eSheetRollSavingThrows(actor) {
  const fromEngine = add2eSheetRollSavingThrowsFromEffectsEngine(actor);
  if (fromEngine) return fromEngine;

  const fallback = actor?.system?.sauvegardes ?? actor?.system?.savingThrows ?? [];
  return {
    saves: Array.isArray(fallback) ? fallback : [],
    progression: null,
    classItem: null,
    classLevel: null,
...
export async function add2eRollSaveCard(actor, idx) {
  if (!actor) return ui.notifications.warn("Aucun acteur pour ce jet.");
  idx = Number(idx);
  const saveData = add2eSheetRollSavingThrows(actor);
  const saves = saveData.saves ?? [];
  ...
  const sourceLabel = saveData.classItem?.name ? ` — ${saveData.classItem.name} niv. ${saveData.classLevel ?? "—"}` : "";
