...
const ARTWORK_VERSION = "2026-06-28-familiar-artwork-v7";
...
function writeActorUpdate(changes, path, value) {
  ...
  changes.system ??= {};
  changes.system.pdv = value;
}

function synchronizeFamiliarCurrentHp(actor, changes = {}, options = {}) {
  if (!options?.add2eFamiliarHpShare && !options?.add2eFamiliarDeathPenalty) return;

  const oldMax = Number(actor?.system?.points_de_coup);
  const oldCurrent = Number(actor?.system?.pdv);
  const nextMax = Number(readActorUpdate(changes, "system.points_de_coup"));
  if (!Number.isFinite(oldMax) || !Number.isFinite(oldCurrent) || !Number.isFinite(nextMax)) return;

  const delta = nextMax - oldMax;
  if (!delta) return;
  writeActorUpdate(changes, "system.pdv", oldCurrent + delta);
}
...
function installArtwork() {
  ...
  Hooks.on("preUpdateActor", synchronizeFamiliarCurrentHp);
