...
const ADD2E_CHAT_CARD_VARIANTS = new Set([
  "neutral", "attack", "damage", "spell", "magic", "ability",
...
function add2eChatCardColorVariables(colors = {}) {
  ...
  return Object.entries(allowed)
    .flatMap(([key, cssVariable]) => {
      const color = String(colors?.[key] ?? "").trim();
      return /^#[0-9a-f]{3,8}$/i.test(color) ? [`${cssVariable}:${color}`] : [];
    })
    .join(";");
}
...
const PARAMETER_LABELS = Object.freeze({
  ...
  duration: "Durée", rounds: "Nombre de rounds", turns: "Nombre de tours", minutes: "Nombre de minutes",
  hours: "Nombre d’heures", days: "Nombre de jours",
  range: "Portée", radius: "Rayon", distance: "Distance", area: "Zone d’effet", zone: "Zone d’effet",
  shape: "Forme de la zone", width: "Largeur", height: "Hauteur", length: "Longueur",
  target: "Cible", targets: "Cibles", recipients: "Bénéficiaires", damage: "Dégâts",
  damageformula: "Formule de dégâts", damagetype: "Type de dégâts", type: "Type", condition: "État",
  effect: "Effet", effects: "Effets", element: "Élément",
  resistance: "Résistance", resistancetype: "Type de résistance", immunity: "Immunité",
  immunitytype: "Type d’immunité", save: "Jet de sauvegarde", savetype: "Type de sauvegarde",
  savemodifier: "Modificateur de sauvegarde",
  spelluuid: "Sort équivalent", spellname: "Nom du sort", school: "École de magie",
  casterlevel: "Niveau du lanceur", activation: "Activation", activationtime: "Temps d’activation",
  trigger: "Déclencheur", mode: "Mode de fonctionnement",
  chargecost: "Coût en charges", charges: "Charges", uses: "Utilisations", frequency: "Fréquence",
  interval: "Intervalle", percentage: "Pourcentage", chance: "Chance", formula: "Formule", table: "Table",
  movementtype: "Type de déplacement", speed: "Vitesse", multiplier: "Multiplicateur",
  armorclass: "Classe d’armure", operation: "Opération", priority: "Priorité", level: "Niveau",
  quantity: "Quantité", count: "Nombre", maximum: "Maximum", minimum: "Minimum", weight: "Poids", size: "Taille",
...
const PARAMETER_HELP = Object.freeze({
  ability: "Choisissez la caractéristique concernée.",
  characteristic: "Choisissez la caractéristique concernée.",
  attribute: "Choisissez la caractéristique concernée.",
  stat: "Choisissez la caractéristique concernée.",
  value: "Indiquez la valeur appliquée par le pouvoir.",
  duration: "Indiquez la durée de l’effet.",
  range: "Indiquez la portée du pouvoir.",
  radius: "Indiquez le rayon de la zone d’effet.",
  distance: "Indiquez la distance concernée.",
  target: "Indiquez la cible du pouvoir.",
  targets: "Indiquez les cibles du pouvoir.",
  damage: "Indiquez les dégâts du pouvoir.",
  damageformula: "Indiquez la formule de dégâts.",
  damagetype: "Choisissez le type de dégâts.",
  save: "Indiquez le jet de sauvegarde applicable.",
  savetype: "Choisissez le type de sauvegarde.",
  spelluuid: "Choisissez le sort équivalent dans le compendium ADD2E.",
  spellname: "Nom du sort équivalent sélectionné.",
  casterlevel: "Indiquez le niveau de lanceur utilisé par le pouvoir.",
  chargecost: "Indiquez le nombre de charges dépensées à chaque utilisation.",
  chance: "Indiquez la chance de réussite en pourcentage.",
  percentage: "Indiquez le pourcentage appliqué.",
  formula: "Indiquez la formule utilisée par le pouvoir.",
  description: "Décrivez l’effet visible ou les précisions utiles."
});
...
const PARAMETER_VALUES = Object.freeze({
  ...
  permanent: "Permanente", automatic: "Automatique", assisted: "Assisté par le MD", manual: "Manuel",
  chat_card: "Carte de chat", none: "Aucun",
  true: "Oui", false: "Non", yes: "Oui", no: "Non",
  self: "Porteur", wearer: "Porteur", equipped: "Équipé", carried: "Transporté",
  ally: "Allié", allies: "Alliés", enemy: "Ennemi", enemies: "Ennemis", all: "Tous",
  configured: "Configurée", passive: "Passive", always_on: "Toujours active",
  activate_or_equipped: "Activation ou équipement",
  add: "Ajouter", additive: "Ajouter", override: "Imposer", set: "Imposer", multiply: "Multiplier",
  round: "Round", rounds: "Rounds", turn: "Tour", turns: "Tours",
  minute: "Minute", minutes: "Minutes", hour: "Heure", hours: "Heures", day: "Jour", days: "Jours",
  fire: "Feu", cold: "Froid", electricity: "Électricité", lightning: "Foudre", acid: "Acide",
  poison: "Poison", magic: "Magie", physical: "Physique", sonic: "Son", necrotic: "Nécrotique",
  radiant: "Rayonnant", slashing: "Tranchant", piercing: "Perforant", bludgeoning: "Contondant",
...
function parameterKey(value) {
  return normalizeUi(value).replace(/_/g, "");
}
...
function parameterHelp(name) {
  return PARAMETER_HELP[parameterKey(name)] ?? "Renseignez la valeur demandée par le pouvoir.";
}

function valueLabel(value, index = null) {
  const raw = String(value ?? "").trim();
  const key = normalizeUi(raw);
  if (Object.hasOwn(PARAMETER_VALUES, key)) return PARAMETER_VALUES[key];
  if (!raw || /^[-+]?\d+(?:[.,]\d+)?$/.test(raw) || /^\d+d\d+(?:[-+]\d+)?$/i.test(raw)) return raw;
  if (/^[\[{]/.test(raw)) return "Paramètres configurés";
...
function characteristic(value) {
  const key = normalizeUi(value);
  for (const [canonical, aliases] of [
    ["force", ["force", "strength", "str"]],
    ["dexterite", ["dexterite", "dexterity", "dex"]],
    ["constitution", ["constitution", "con"]],
    ["intelligence", ["intelligence", "int"]],
    ["sagesse", ["sagesse", "wisdom", "wis"]],
    ["charisme", ["charisme", "charisma", "cha"]]
  ]) {
...
function ensureGlobalStyles() {
  ...
  const id = "add2e-global-ui-style";
  const old = document.getElementById(id);
  if (old?.dataset?.version === ADD2E_GLOBAL_UI_VERSION) return;
  old?.remove();

  const style = document.createElement("style");
  style.id = id;
  style.dataset.version = ADD2E_GLOBAL_UI_VERSION;
  style.textContent = `
...
function labelOf(control) {
  ...
  return group.matches?.("label.form-group")
    ? group.querySelector(":scope > span")
    : group.querySelector(":scope > label") ?? group.querySelector("label");
}

function localizeLabel(control) {
  if (!control || control.dataset.add2eFrenchLabel === "1") return;
  const label = labelOf(control);
  if (!label) return;
  const required = label.textContent?.includes("*") === true;
  label.textContent = parameterLabel(control.name);
  if (required) {
    const star = document.createElement("span");
    star.textContent = " *";
    star.style.color = "#a40000";
    label.append(star);
  }
  control.dataset.add2eFrenchLabel = "1";
}

function addHelp(control) {
  const group = groupOf(control);
  if (!group || [...group.querySelectorAll(".add2e-parameter-help")].some(node => node.dataset.parameter === control.name)) return;
  const help = document.createElement("p");
  help.className = "add2e-parameter-help";
  help.dataset.parameter = control.name;
  help.textContent = parameterHelp(control.name);
  group.append(help);
}

function localizeOptions(select) {
  if (!select || select.dataset.add2eFrenchOptions === "1") return;
  [...select.options].forEach((option, index) => {
    if (!option.value) return;
    const key = normalizeUi(option.value);
    if (Object.hasOwn(PARAMETER_VALUES, key)) option.textContent = PARAMETER_VALUES[key];
    else if (!String(option.textContent ?? "").trim() || String(option.textContent).trim() === String(option.value)) {
      option.textContent = valueLabel(option.value, index);
    }
  });
  select.dataset.add2eFrenchOptions = "1";
}

function replaceCharacteristic(form) {
  const current = form.querySelector('[name="ability"],[name="characteristic"],[name="attribute"],[name="stat"]');
  if (!current) return null;
  if (current instanceof HTMLSelectElement && current.dataset.add2eFrenchCharacteristic === "1") return current;
  const select = document.createElement("select");
  for (const attribute of current.attributes) {
    if (!["name", "type", "value"].includes(attribute.name)) select.setAttribute(attribute.name, attribute.value);
  }
  ...
  select.value = characteristic(current.value);
  current.replaceWith(select);
  localizeLabel(select);
  addHelp(select);
  return select;
}

function replaceCharacteristicValue(form) {
  const characteristicControl = form.querySelector('[name="ability"],[name="characteristic"],[name="attribute"],[name="stat"]');
  const current = form.querySelector('[name="value"]');
  if (!characteristicControl || !current) return current;
  if (current instanceof HTMLInputElement && current.type === "number" && current.dataset.add2eFrenchAbilityValue === "1") return current;
  const input = document.createElement("input");
  for (const attribute of current.attributes) {
    if (!["type", "value", "rows"].includes(attribute.name)) input.setAttribute(attribute.name, attribute.value);
  }
  input.type = "number";
  input.step = "1";
...
function localizeDuration(form) {
  ...
  visible.type = "text";
  visible.value = normalizeUi(current.value) === "permanent" ? "Permanente" : String(current.value ?? "");
  ...
  const sync = () => {
    ...
    current.value = normalizeUi(value) === "permanente" ? "permanent" : value;
  };
  visible.addEventListener("input", sync);
  current.insertAdjacentElement("afterend", visible);
  sync();
  localizeLabel(current);
  addHelp(current);
}

function localizeFixed(control) {
  const type = normalizeUi(control?.dataset?.add2eParameterType);
  if (!["fixed", "fixed_list"].includes(type) || control.dataset.add2eFrenchFixed === "1") return;
  const display = [...(groupOf(control)?.children ?? [])]
    .find(element => element !== control && element.tagName !== "LABEL" && !element.classList?.contains("add2e-parameter-help"));
  if (!display) return;
  ...
  display.textContent = raw
    ? values.map((value, index) => valueLabel(value, index)).join(", ")
    : "Valeur définie par le pouvoir";
  control.dataset.add2eFrenchFixed = "1";
}

function localizeForm(form) {
  for (const control of form.querySelectorAll('[data-add2e-parameter-type][name]')) {
    localizeLabel(control);
    if (control instanceof HTMLSelectElement) localizeOptions(control);
    localizeFixed(control);
    addHelp(control);
  }
  replaceCharacteristic(form);
  replaceCharacteristicValue(form);
  localizeDuration(form);
  const finalHelp = form.querySelector(":scope > p:last-child");
  if (finalHelp && !finalHelp.classList.contains("add2e-parameter-help") && finalHelp.dataset.add2eFrenchFinalHelp !== "1") {
    finalHelp.textContent = "Les paramètres marqués d’un astérisque sont obligatoires. Les champs complexes acceptent une valeur simple ou une structure JSON.";
    finalHelp.dataset.add2eFrenchFinalHelp = "1";
  }
}
...
function missingRequired(form) {
  const missing = [];
  for (const control of form.querySelectorAll('[data-add2e-parameter-type][name]')) {
    if (!labelOf(control)?.textContent?.includes("*")) continue;
    ...
    if (value === false || value === "" || (Array.isArray(value) && !value.length)) {
      missing.push(parameterLabel(control.name));
    }
  }
  return [...new Set(missing)];
}

function bindValidation(root, form) {
  if (root.dataset.add2eMagicFrenchValidation === "1") return;
  root.dataset.add2eMagicFrenchValidation = "1";
  root.addEventListener("click", event => {
    const button = event.target instanceof Element
      ? event.target.closest('button[data-action="save"]')
      : null;
    if (!button) return;
...
function localizeSummary(text) {
  return String(text ?? "").split(" · ").map(entry => {
    const separator = entry.indexOf(":");
    if (separator < 0) return entry;
    return `${parameterLabel(entry.slice(0, separator).trim())} : ${valueLabel(entry.slice(separator + 1).trim(), 0)}`;
  }).join(" · ");
}

function localizeSummaries(form) {
  for (const summary of form.querySelectorAll("[data-add2e-selected-powers] small")) {
    if (summary.dataset.add2eFrenchSummary === "1") continue;
    summary.textContent = localizeSummary(summary.textContent);
    summary.dataset.add2eFrenchSummary = "1";
  }
}

function observeCreator(form) {
  localizeSummaries(form);
  ...
  new MutationObserver(() => localizeSummaries(form))
    .observe(container, { childList: true, subtree: true, characterData: true });
}

function enhanceCanonicalPowerWindow(app, html) {
  const root = rootElement(app, html);
  if (!root) return;
  const creator = root.matches?.(".add2e-magic-item-create-form")
    ? root
    : root.querySelector?.(".add2e-magic-item-create-form");
  if (creator) observeCreator(creator);
  const form = root.matches?.(".add2e-magic-power-parameter-form")
    ? root
    : root.querySelector?.(".add2e-magic-power-parameter-form");
  if (!form) return;
  const application = form.closest?.(".application")
    ?? (root.matches?.(".application") ? root : null)
    ?? (app?.element instanceof HTMLElement ? app.element : app?.element?.[0]);
  if (application instanceof HTMLElement) {
    ensureGlobalStyles();
    application.classList.add("add2e-system-window", "add2e-magic-power-window");
  }
  localizeForm(form);
  bindValidation(application ?? root, form);
  if (form.dataset.add2eCanonicalObserver !== "1") {
    form.dataset.add2eCanonicalObserver = "1";
    new MutationObserver(() => localizeForm(form))
      .observe(form, { childList: true, subtree: true });
  }
}

function installGlobalUiHooks() {
  if (globalThis.__ADD2E_GLOBAL_UI_HOOKS_V1__) return;
  globalThis.__ADD2E_GLOBAL_UI_HOOKS_V1__ = true;
  Hooks.on("renderDialogV2", enhanceCanonicalPowerWindow);
  Hooks.on("renderApplicationV2", enhanceCanonicalPowerWindow);
}
