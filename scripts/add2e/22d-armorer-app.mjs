...
class Add2eArmorerApp extends foundry.applications.api.ApplicationV2 {
  ...
  async _renderHTML(context) {
    ...
    root.innerHTML = `${STYLE}<p><span>Acheteur :</span> ${buyer} <strong>${this.buyer ? esc(formatMoney(getMoney(this.buyer))) : ""}</strong></p><div>${button("all", "Tous")}${button("weapons", "Armes")}${button("armors", "Armures")}${button("projectiles", "Projectiles")}${restock}</div><input class="search" value="${esc(this.search)}" placeholder="Recherche"><div class="add2e-armorer-scroll"><table class="add2e-armorer-table"><thead><tr><th class="col-article">Article</th><th class="col-type">Type</th><th class="col-price">Prix</th><th class="col-stock">Stock</th><th class="col-qty">Qté</th><th class="col-action"></th></tr></thead><tbody>${rows}</tbody></table></div>`;
    return root;
  }
