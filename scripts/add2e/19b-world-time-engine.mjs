...
// Version : 2026-06-06-world-time-engine-v8-xp-toolbar-pattern
//
// Rôle :
...
export const ADD2E_WORLD_TIME_ENGINE_VERSION = "2026-06-06-world-time-engine-v8-xp-toolbar-pattern";
...
export async function add2eWorldTimeExpireAllActors({ reason = "world-time", currentRound = null } = {}) {
  ...
  for (const actor of allWorldActors()) {
    try {
      ...
      console.error(`${TAG}[ACTOR_EXPIRE_FAILED]`, { actor: actor.name, actorId: actor.id, err });
      rows.push({ actor: actor.name, actorId: actor.id, error: String(err?.message || err) });
    }
  }
...
export function add2eRegisterWorldTimeEngine() {
  ...
  registerToolbarHook();
  log("[REGISTERED]", { version: ADD2E_WORLD_TIME_ENGINE_VERSION, tick: add2eTimeCurrentTick(), toolbar: "xp-pattern" });
  return true;
}
