...
const ADD2E_ABILITY_DERIVED_VERSION = "2026-07-26-canonical-derived-abilities-3-25-v2";
...
// ===============================
// PROFILS DÉRIVÉS CANONIQUES 3–25
// ===============================
...
export const ADD2E_ABILITY_BOUNDS = Object.freeze(Object.fromEntries(
  ["force", "dexterite", "constitution", "intelligence", "sagesse", "charisme"]
    .map(ability => [ability, Object.freeze({ min: ADD2E_ABILITY_MIN, max: ADD2E_ABILITY_MAX })])
));
...
function add2eNormalizeDerivedAbility(value) {
  const key = String(value ?? "")
    ...
    .replace(/[^a-z0-9]+/g, "");
  return ({
    str: "force", strength: "force",
    dex: "dexterite", dexterity: "dexterite",
    con: "constitution", int: "intelligence",
    wis: "sagesse", wisdom: "sagesse",
    cha: "charisme", charisma: "charisme"
  })[key] ?? key;
}
  ...
  return Array.from(actor?.items ?? [])
    ...
    .some(item => {
      const values = [item?.name, item?.system?.slug, item?.system?.nom, item?.system?.name, item?.system?.label]
        .map(add2eNormalizeClassForExceptionalStrength)
        .filter(Boolean);
...
function add2eDerivedProfileDirective(resolution) {
  ...
  const rawProfile = metadata.profile && typeof metadata.profile === "object" && !Array.isArray(metadata.profile)
    ? foundry.utils.deepClone(metadata.profile)
    : {};
...
function add2eResolveAbilityDerived(actor, ability, context = {}) {
  ...
  const profile = directive.mode === "table"
    ? foundry.utils.deepClone(baseProfile)
