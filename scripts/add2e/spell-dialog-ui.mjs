...
function spellLevel(value) {
  return Math.max(1, Number(value?.level ?? value?.niveau ?? value?.spellLevel ?? value?.system?.niveau ?? value?.system?.level ?? value?.system?.niveau_sort ?? value?.system?.spellLevel ?? 1) || 1);
}
...
async function resolveCanonicalSpell(spell) {
  const sourceUuid = String(spell?.flags?.core?.sourceId ?? spell?._stats?.compendiumSource ?? spell?.flags?.add2e?.sourceUuid ?? "").trim();
...
async function openPlayerSpellbook(actor, book) {
  ...
  const actorSpells = knownActorSpells(actor, book);
  if (!actorSpells.length) return ui.notifications.info(`${book.name} ne contient aucun sort connu.`);
  const detailed = [];
  for (const embedded of actorSpells) {
