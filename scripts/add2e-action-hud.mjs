...
// Version : 2026-06-01-v40-drag-handle-pointer-mouse
// Principe : le HUD est une interface. Les jets et actions délèguent strictement aux fonctions de la feuille/système.

const ADD2E_ACTION_HUD_VERSION = "2026-06-01-v40-drag-handle-pointer-mouse";
const HUD_ID = "add2e-action-hud";
const STYLE_ID = "add2e-action-hud-style";
const STORAGE_KEY = "add2e.actionHud.state.v40";
const LEGACY_STORAGE_KEYS = ["add2e.actionHud.state.v39", "add2e.actionHud.state.v34", "add2e.actionHud.layout.v1"];
...
#${HUD_ID} .a2e-hud-header{display:grid;grid-template-columns:74px minmax(0,1fr) auto auto auto;gap:10px;align-items:center;padding:9px 10px;background:linear-gradient(180deg,rgba(78,48,18,.78),rgba(36,26,14,.62));cursor:move}
...
#${HUD_ID} .icon{width:30px;height:30px;border:1px solid rgba(214,176,90,.75);border-radius:9px;background:rgba(255,244,201,.12);color:#ffe4a1;cursor:pointer}#${HUD_ID} .drag{cursor:move!important}#${HUD_ID} .resize{cursor:nwse-resize!important}
...
function isPrimaryButtonEvent(ev) { return ev.button === undefined || ev.button === 0; }
function startResize(ev) { const handle = ev.target?.closest?.("[data-resize-handle]"); const el = ev.target?.closest?.(`#${HUD_ID}`); if (!handle || !el || !isPrimaryButtonEvent(ev)) return false; ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation?.(); resizing = true; const moveEvent = ev.type === "mousedown" ? "mousemove" : "pointermove"; const upEvent = ev.type === "mousedown" ? "mouseup" : "pointerup"; const startState = loadState(); const start = { x: ev.clientX, y: ev.clientY, width: startState.width, maxMenuHeight: startState.maxMenuHeight, left: startState.left, top: startState.top }; const move = e => { e.preventDefault?.(); const s = loadState(); s.width = clamp(start.width + e.clientX - start.x, 420, Math.max(440, window.innerWidth - start.left - 8)); s.maxMenuHeight = clamp(start.maxMenuHeight + e.clientY - start.y, 140, Math.max(160, window.innerHeight - start.top - 48)); applyGeometry(el, true); }; const up = () => { window.removeEventListener(moveEvent, move, true); window.removeEventListener(upEvent, up, true); resizing = false; const s = loadState(); saveState({ width: el.offsetWidth || s.width, maxMenuHeight: s.maxMenuHeight, left: s.left, top: s.top }); applyGeometry(el, true); }; window.addEventListener(moveEvent, move, true); window.addEventListener(upEvent, up, true); return true; }
function startDrag(ev) { if (dragging || resizing) return false; const el = ev.target?.closest?.(`#${HUD_ID}`); const handle = ev.target?.closest?.("[data-drag-handle]"); const interactive = ev.target?.closest?.("button,a,input,select,textarea,[data-action],[data-tab],[data-resize-handle]"); if (!el || !handle || !isPrimaryButtonEvent(ev) || (interactive && !interactive.matches?.("[data-drag-handle]"))) return false; ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation?.(); dragging = true; const moveEvent = ev.type === "mousedown" ? "mousemove" : "pointermove"; const upEvent = ev.type === "mousedown" ? "mouseup" : "pointerup"; const s0 = loadState(); const start = { x: ev.clientX, y: ev.clientY, left: s0.left, top: s0.top }; console.log(`${TAG}[DRAG_START]`, { version: ADD2E_ACTION_HUD_VERSION, eventType: ev.type, left: start.left, top: start.top }); const move = e => { e.preventDefault?.(); const s = loadState(); const width = el.offsetWidth || s.width; const height = el.offsetHeight || 110; s.left = clamp(start.left + e.clientX - start.x, 8, Math.max(8, window.innerWidth - width - 8)); s.top = clamp(start.top + e.clientY - start.y, 8, Math.max(8, window.innerHeight - Math.min(height, 48))); applyGeometry(el, true); saveState({ left: Math.round(s.left), top: Math.round(s.top), bottom: null }); }; const up = () => { window.removeEventListener(moveEvent, move, true); window.removeEventListener(upEvent, up, true); dragging = false; applyGeometry(el, true); }; window.addEventListener(moveEvent, move, true); window.addEventListener(upEvent, up, true); return true; }
