...
function configureInitiative() {
  ...
  console.log(`${TAG}[CONFIG]`, { version: ADD2E_INITIATIVE_VERSION, refresh: ADD2E_INITIATIVE_REFRESH_SYNC_VERSION, formula: "1d6", order: "ascending", rule: "low-first" });
}
...
function safeClone(value) { try { return globalThis.foundry?.utils?.deepClone ? globalThis.foundry.utils.deepClone(value) : JSON.parse(JSON.stringify(value)); } catch (_e) { return value ?? null; } }
...
function add2eInitiativeDebug(label, combat = game.combat, extra = {}) {
  try {
    ...
    const nativeCombatant = combat?.combatant ?? null;
    const selected = Array.from(canvas?.tokens?.controlled ?? []).map(t => `${t.name}:${t.id}:${t.actor?.id ?? "no-actor"}`);
    console.log(`${TAG}[DEBUG][${label}]`, {
      version: ADD2E_INITIATIVE_VERSION,
      refreshSync: ADD2E_INITIATIVE_REFRESH_SYNC_VERSION,
      ...
      activeByScript: active ? { id: active.id, name: active.name, initiative: active.initiative, tokenId: active.tokenId, actorId: active.actorId ?? active.actor?.id ?? null } : null,
      activeByFoundry: nativeCombatant ? { id: nativeCombatant.id, name: nativeCombatant.name, initiative: nativeCombatant.initiative, tokenId: nativeCombatant.tokenId, actorId: nativeCombatant.actorId ?? nativeCombatant.actor?.id ?? null } : null,
      sorted: sorted.map((c, i) => `${i}:${c.name}:${c.id}:init=${c.initiative}:sort=${c.sort}:token=${c.tokenId}`),
      turns: turns.map((c, i) => `${i}:${c.name}:${c.id}:init=${c.initiative}:sort=${c.sort}:token=${c.tokenId}`),
      selected,
      extra
    });
  } catch (err) { console.warn(`${TAG}[DEBUG][${label}][ERROR]`, err); }
}
...
function applyAscendingTurns(combat = game.combat, { forceFirst = false, reason = "apply" } = {}) {
  ...
  try {
    ...
    console.log(`${TAG}[APPLY_TURNS]`, { reason, started: combat.started, forceFirst, index: combat.started || forceFirst ? index : null, active: combat.started || forceFirst ? turns[index]?.name : null, order: turns.map(c => `${c.name}:${c.initiative}`) });
    return true;
...
function selectActiveCombatantToken(combat = game.combat, { reason = "sync" } = {}) {
  if (!combat?.started) { console.log(`${TAG}[TOKEN_SYNC][SKIP_NOT_STARTED]`, { reason }); return false; }
  const combatant = currentCombatant(combat);
  const token = tokenFromCombatant(combatant);
  if (!token?.control) { console.warn(`${TAG}[TOKEN_SYNC][NO_TOKEN]`, { reason, combatant: combatant?.name ?? null, combatantId: combatant?.id ?? null, tokenId: combatant?.tokenId ?? null }); return false; }
  try {
    ...
    console.log(`${TAG}[TOKEN_SYNC]`, { reason, combatant: combatant?.name ?? null, combatantId: combatant?.id ?? null, token: token?.name ?? null, tokenId: token?.id ?? null });
    return true;
  } catch (err) { console.warn(`${TAG}[TOKEN_SYNC][ERROR]`, err); return false; }
}

async function syncCombatAfterRefresh(combat = game.combat, { reason = "refresh", render = true, selectToken = true } = {}) {
  if (!combat || refreshSyncing) { console.warn(`${TAG}[REFRESH_SYNC][SKIP]`, { hasCombat: !!combat, refreshSyncing, reason }); return false; }
  const sorted = sortedCombatants(combat);
  if (!sorted.length) return false;
  refreshSyncing = true;
  try {
    add2eInitiativeDebug(`REFRESH_BEFORE:${reason}`, combat, { render, selectToken });
    combat.setupTurns?.();
    combat.turns = sorted;

    if (!combat.started) {
      console.log(`${TAG}[REFRESH_SYNC][NOT_STARTED]`, { reason, order: sorted.map(c => `${c.name}:${c.initiative}`) });
    ...
    console.log(`${TAG}[REFRESH_SYNC]`, { version: ADD2E_INITIATIVE_REFRESH_SYNC_VERSION, reason, round: combat.round, turn: index, active: active?.name ?? null, activeId: active?.id ?? null, updatedTurn: needsUpdate, order: sorted.map(c => `${c.name}:${c.initiative}`) });
    add2eInitiativeDebug(`REFRESH_AFTER:${reason}`, combat, { index, activeId: active?.id ?? null, updatedTurn: needsUpdate });
    return true;
  } catch (err) { console.warn(`${TAG}[REFRESH_SYNC][ERROR]`, err); return false; }
  finally { refreshSyncing = false; }
}
function scheduleRefreshSync(combat = game.combat, { reason = "refresh", delay = 120, render = true, selectToken = true } = {}) {
  if (!combat) return;
  console.log(`${TAG}[REFRESH_SYNC][SCHEDULE]`, { reason, delay, render, selectToken, combatId: combat?.id ?? null, started: combat?.started ?? null, turn: combat?.turn ?? null, current: combat?.current ?? null });
...
async function updateCombatTurnBySortedIndex(combat, newIndex, { roundDelta = 0 } = {}) {
  ...
  scheduleClearMovementTrail();
  console.log(`${TAG}[NAV]`, { round, index, target: target?.name ?? null, targetId: target?.id ?? null, order: turns.map(c => `${c.name}:${c.initiative}`) });
  return combat;
}
async function forceFirstSortedTurn(combat = game.combat, reason = "start") {
  ...
  scheduleHudFollow(combat, { forceOpen: false, reason });
  scheduleClearMovementTrail();
  console.log(`${TAG}[START_FIRST]`, { reason, first: turns[0]?.name ?? null, firstId: turns[0]?.id ?? null, order: turns.map(c => `${c.name}:${c.initiative}`) });
  return combat;
}
...
function installCombatPatch() {
  ...
  console.log(`${TAG}[COMBAT_PATCH]`, { initiative: ADD2E_INITIATIVE_VERSION, refresh: ADD2E_INITIATIVE_REFRESH_SYNC_VERSION });
  return true;
}
...
async function sortInitiativeAscending(combat = game.combat) {
  ...
  try {
    ...
    console.log(`${TAG}[SORT_ASC_REAL]`, { started: combat.started, updates, order: sorted.map(c => `${c.name}:${c.initiative}`) });
    return true;
...
function escapeHtml(value) { try { return globalThis.foundry?.utils?.escapeHTML ? globalThis.foundry.utils.escapeHTML(String(value ?? "")) : String(value ?? ""); } catch (_e) { const div = document.createElement("div"); div.textContent = String(value ?? ""); return div.innerHTML; } }
...
function hasProperty(obj, path) { return globalThis.foundry?.utils?.hasProperty ? globalThis.foundry.utils.hasProperty(obj, path) : path in (obj ?? {}); }
function isTokenPositionUpdate(changes = {}) { return hasProperty(changes, "x") || hasProperty(changes, "y") || hasProperty(changes, "elevation") || hasProperty(changes, "rotation"); }
...
function installMovementHistoryBlock() { if (movementHistoryPatchInstalled) return true; movementHistoryPatchInstalled = true; const patched = []; if (patchSetFlagHistoryBlock(globalThis.Actor, "Actor")) patched.push("Actor"); if (patchSetFlagHistoryBlock(globalThis.TokenDocument ?? globalThis.foundry?.documents?.TokenDocument, "TokenDocument")) patched.push("TokenDocument"); cleanupMovementHistoryHooks(); installTokenHoverPatch(); game.add2e = game.add2e ?? {}; game.add2e.movementHistoryDisabledVersion = ADD2E_MOVEMENT_HISTORY_DISABLED_VERSION; console.log(`${TAG}[MOVE_HISTORY][DISABLED]`, { version: ADD2E_MOVEMENT_HISTORY_DISABLED_VERSION, patched }); return true; }
...
function exposeGlobals() { game.add2e = game.add2e ?? {}; game.add2e.initiativeVersion = ADD2E_INITIATIVE_VERSION; game.add2e.initiativeHudFollowVersion = ADD2E_INITIATIVE_HUD_FOLLOW_VERSION; game.add2e.initiativeRefreshSyncVersion = ADD2E_INITIATIVE_REFRESH_SYNC_VERSION; game.add2e.movementHistoryDisabledVersion = ADD2E_MOVEMENT_HISTORY_DISABLED_VERSION; game.add2e.individualTurnLockVersion = ADD2E_INDIVIDUAL_TURN_LOCK_VERSION; globalThis.add2eConfigureInitiative = configureInitiative; globalThis.add2eSortInitiativeAscending = sortInitiativeAscending; globalThis.add2eScheduleInitiativeSort = scheduleInitiativeSort; globalThis.add2ePatchCombatTrackerInitiativeIcons = patchInitiativeIcons; globalThis.add2eCanActorActNow = canActorActNow; globalThis.add2eCanTokenInteractNow = canTokenInteractNow; globalThis.add2eSyncActionHudToCombatant = syncActionHudToCombatant; globalThis.add2eSyncCombatAfterRefresh = syncCombatAfterRefresh; globalThis.add2eScheduleRefreshSync = scheduleRefreshSync; globalThis.add2eDebugCombatState = add2eInitiativeDebug; globalThis.add2eSelectActiveCombatantToken = selectActiveCombatantToken; globalThis.add2eDisableMovementHistoryRecording = installMovementHistoryBlock; globalThis.add2eInstallIndividualTurnLock = installTokenInteractionLock; globalThis.add2eClearExistingMovementHistoryFlags = clearExistingMovementHistoryFlags; globalThis.add2eCleanupMovementHistoryHooks = cleanupMovementHistoryHooks; globalThis.add2eClearFoundryMovementTrail = clearFoundryMovementTrailAggressive; globalThis.add2eInstallTokenHoverPatch = installTokenHoverPatch; globalThis.add2eForceFirstSortedTurn = forceFirstSortedTurn; globalThis.triInitiativeAscendant = sortInitiativeAscending; }
function installHooks() {
  Hooks.on("preUpdateToken", (tokenDoc, changes, options, userId) => { if (options?.add2eIgnoreTurnLock || !isTokenPositionUpdate(changes)) return; const user = game.users?.get?.(userId); if (user?.isGM) return; if (!blockOutOfTurnTokenInteraction(tokenDoc, { notify: game.user?.id === userId })) return false; });
  Hooks.on("updateCombatant", (combatant, changes, options) => { console.log(`${TAG}[HOOK:updateCombatant]`, { name: combatant?.name ?? null, id: combatant?.id ?? null, changes, options }); if (options?.add2eInitiativeSort) return; if (hasProperty(changes ?? {}, "initiative")) scheduleInitiativeSort(combatant.combat ?? game.combat); });
  Hooks.on("createCombatant", (combatant, options) => { console.log(`${TAG}[HOOK:createCombatant]`, { name: combatant?.name ?? null, id: combatant?.id ?? null, options }); if (!options?.add2eInitiativeSort) scheduleInitiativeSort(combatant.combat ?? game.combat); });
  Hooks.on("deleteCombatant", (combatant, options) => { console.log(`${TAG}[HOOK:deleteCombatant]`, { name: combatant?.name ?? null, id: combatant?.id ?? null, options }); if (!options?.add2eInitiativeSort) scheduleInitiativeSort(combatant.combat ?? game.combat); });
  Hooks.on("updateCombat", (combat, changes, options) => { console.log(`${TAG}[HOOK:updateCombat]`, { changes, options, combatId: combat?.id ?? null, started: combat?.started ?? null, turn: combat?.turn ?? null, current: combat?.current ?? null }); if (options?.add2eInitiativeSort || options?.add2eRefreshSync) return; if (hasProperty(changes ?? {}, "started") && combat?.started) { setTimeout(() => forceFirstSortedTurn(combat, "updateCombat-started"), 80); return; } if (hasProperty(changes ?? {}, "turn") || hasProperty(changes ?? {}, "round") || hasProperty(changes ?? {}, "current")) { applyAscendingTurns(combat, { reason: "updateCombat" }); if (combat?.started) scheduleRefreshSync(combat, { reason: "updateCombat", delay: 80 }); scheduleHudFollow(combat, { forceOpen: false, reason: "updateCombat" }); scheduleClearMovementTrail(); ui.combat?.render?.(true); } });
  Hooks.on("combatTurn", combat => { add2eInitiativeDebug("HOOK_COMBAT_TURN", combat); scheduleRefreshSync(combat, { reason: "combatTurn", delay: 40 }); scheduleHudFollow(combat, { forceOpen: false, reason: "combatTurn" }); scheduleClearMovementTrail(); });
  Hooks.on("combatRound", combat => { add2eInitiativeDebug("HOOK_COMBAT_ROUND", combat); scheduleRefreshSync(combat, { reason: "combatRound", delay: 40 }); scheduleHudFollow(combat, { forceOpen: false, reason: "combatRound" }); scheduleClearMovementTrail(); });
  ...
  Hooks.on("controlToken", token => { console.log(`${TAG}[HOOK:controlToken]`, { token: token?.name ?? null, id: token?.id ?? null, controlled: token?.controlled ?? null }); clearFoundryMovementTrailAggressive(token); });
  Hooks.on("refreshToken", token => clearFoundryMovementTrailAggressive(token));
  Hooks.on("canvasReady", () => { add2eInitiativeDebug("HOOK_CANVAS_READY", game.combat); installTokenHoverPatch(); installTokenInteractionLock(); scheduleRefreshSync(game.combat, { reason: "canvasReady", delay: 180, selectToken: game.combat?.started }); scheduleClearMovementTrail(); });
  Hooks.on("deleteCombat", () => { lastHudCombatantKey = ""; scheduleClearMovementTrail(); });
  Hooks.on("renderCombatTracker", (app, html) => { add2eInitiativeDebug("HOOK_RENDER_COMBAT_TRACKER", game.combat, { appId: app?.id ?? app?.options?.id ?? null }); patchInitiativeIcons(html); scheduleRefreshSync(game.combat, { reason: "renderCombatTracker", delay: 20, render: false, selectToken: game.combat?.started }); });
  Hooks.on("renderCombatantConfig", () => setTimeout(() => patchInitiativeIcons(document), 50));
  Hooks.on("renderSidebarTab", (app, html) => { if (app?.options?.id === "combat" || app?.tabName === "combat" || app?.id === "combat") { add2eInitiativeDebug("HOOK_RENDER_SIDEBAR_COMBAT", game.combat, { appId: app?.id ?? app?.options?.id ?? null }); patchInitiativeIcons(html); scheduleRefreshSync(game.combat, { reason: "renderSidebarTab", delay: 30, render: false, selectToken: game.combat?.started }); setTimeout(() => patchInitiativeIcons(document), 50); } });
  installInitiativeChatCard();
  console.log(`${TAG}[HOOKS_INSTALLED]`, { version: ADD2E_INITIATIVE_VERSION, hudFollow: ADD2E_INITIATIVE_HUD_FOLLOW_VERSION, refreshSync: ADD2E_INITIATIVE_REFRESH_SYNC_VERSION, movementHistoryDisabled: ADD2E_MOVEMENT_HISTORY_DISABLED_VERSION, individualTurnLock: ADD2E_INDIVIDUAL_TURN_LOCK_VERSION });
}
...
Hooks.once("ready", () => {
  ...
  exposeGlobals();
  add2eInitiativeDebug("READY_AFTER_EXPOSE", game.combat);
  ...
  setTimeout(() => add2eInitiativeDebug("READY_DIAGNOSTIC_2000", game.combat), 2000);
