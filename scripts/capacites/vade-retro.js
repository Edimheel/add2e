...
// Une utilisation par combat. La visée reprend exactement le flux de Mains Brûlantes.

const ADD2E_VADE_RETRO_VERSION = "2026-07-05-mains-brulantes-crosshairs-v10";
...
return await (async () => {
  ...
  let casterToken = canvas.tokens?.controlled?.find(token => token?.actor?.id === caster.id) ?? null;
  if (!casterToken) {
    const tokens = caster.getActiveTokens?.() ?? [];
    if (tokens.length > 0) casterToken = tokens[0];
  }
  ...
  // ==============================================================
  // VISÉE — bloc volontairement identique à Mains Brûlantes.
  // ==============================================================
  ...
  const coneDistance = caseDistance * 3;
  const coneAngle = 90;
  const config = {
    size: 1,
  ...
  const templateX = Number(templateObject?.x ?? origin.x);
  const templateY = Number(templateObject?.y ?? origin.y);
  const tokenTouchesCone = token => {
    const points = [
      ...
      { x: token.x + token.w / 2, y: token.y },
      { x: token.x + token.w / 2, y: token.y + token.h },
      { x: token.x, y: token.y + token.h / 2 },
      { x: token.x + token.w, y: token.y + token.h / 2 }
    ];
  ...
  setTimeout(() => createdTemplate.delete(), 1200);
  ...
  if (attempted && usageFlagKey) {
    await caster.setFlag("add2e", usageFlagKey, {
      ...
      poolRoll: poolRoll.total,
      remainingDV,
      affectedTargets: rows
        .filter(row => ["Repoussé", "Détruit / damné", "Influencé", "Dominé"].includes(row.result))
        .map(row => row.target),
      at: Date.now()
    });
  }
  ...
  await ChatMessage.create({
    ...
    content: `<div class="add2e-chat-card" style="border:1.5px solid #c79b38;border-radius:12px;overflow:hidden;background:#fffaf0;box-shadow:0 3px 8px #0002;"><div style="background:linear-gradient(90deg,#8a5a13,#d4a83a);color:white;padding:8px 10px;display:flex;align-items:center;gap:8px;"><img src="${escapeHtml(caster.img)}" style="width:36px;height:36px;border-radius:50%;border:2px solid #fff;object-fit:cover;"><div style="flex:1;"><div style="font-weight:800;">${escapeHtml(caster.name)}</div><div style="font-size:.9em;">Vade-rétro — niveau effectif de clerc ${level}${isPaladin ? ` (Paladin niveau ${nativeLevel})` : ""}</div></div></div><div style="padding:8px;"><div style="margin-bottom:7px;padding:6px 8px;border:1px solid #e7d8a0;border-radius:6px;background:#fffdf7;"><b>Cône :</b> ${coneDistance} ${escapeHtml(canvas.scene?.grid?.units || "m")}, ${coneAngle}°. <b>Pool :</b> 2d6 = <b>${poolRoll.total} DV</b> ; restant : <b>${remainingDV} DV</b>.</div>${rows.map(row => `<div style="border-bottom:1px solid #e7d8a0;padding:5px 0;"><b>${escapeHtml(row.target)}</b> — <b>${escapeHtml(row.result)}</b><br><span style="font-size:.9em;color:#5b4b26;">${escapeHtml(row.detail)}</span></div>`).join("") || "<div>Aucune créature n’a pu être affectée.</div>"}<div style="margin-top:7px;font-size:.82em;color:#5b4b26;">Vade-rétro : une utilisation par combat.</div></div></div>`
  });
