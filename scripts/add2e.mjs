...
function add2eResolveMonsterSheetImage(actor) {
  const defaultImg = "icons/svg/mystery-man.svg";
  const actorImg = String(actor?.img ?? "").trim();
  const tokenImg = String(foundry.utils.getProperty(actor ?? {}, "prototypeToken.texture.src") ?? "").trim();
  const defaultImages = new Set(["", defaultImg, "icons/svg/mystery-man-black.svg"]);

  if (!defaultImages.has(actorImg)) return actorImg;
  if (!defaultImages.has(tokenImg)) return tokenImg;
  return actorImg || tokenImg || defaultImg;
}

function add2eInstallMonsterSheetPresentationPatch() {
  if (globalThis.__ADD2E_MONSTER_SHEET_PRESENTATION_PATCH_V1__) return;
  globalThis.__ADD2E_MONSTER_SHEET_PRESENTATION_PATCH_V1__ = true;

  const MonsterSheetClass = globalThis.Add2eMonsterSheet;
  if (!MonsterSheetClass?.prototype) return;

  MonsterSheetClass.DEFAULT_OPTIONS.position = {
    ...(MonsterSheetClass.DEFAULT_OPTIONS.position ?? {}),
    width: 980,
    height: 850
  };
  ...
  if (typeof originalGetData === "function" && MonsterSheetClass.prototype.__add2eMonsterSheetImagePatch !== true) {
    MonsterSheetClass.prototype.__add2eMonsterSheetImagePatch = true;
    MonsterSheetClass.prototype.getData = async function add2eMonsterSheetGetDataWithImage(...args) {
      const data = await originalGetData.apply(this, args);
      data.img = add2eResolveMonsterSheetImage(this.actor);
      return data;
    };
  }

  const styleId = "add2e-monster-sheet-presentation-patch";
  if (!document.getElementById(styleId)) {
    const style = document.createElement("style");
    style.id = styleId;
    style.textContent = `
      .application.add2e.sheet.actor.monster,
      .window-app.add2e.sheet.actor.monster {
        min-width: 980px !important;
      }
      .add2e.sheet.monster .add2e-monster-readable-sheet {
        min-width: 980px !important;
      }
      .add2e.sheet.monster .monster-header {
        grid-template-columns: 180px 1fr !important;
      }
      .add2e.sheet.monster .profile-img img {
      ...
      .add2e.sheet.monster .meta-grid,
      .add2e.sheet.monster .key-stats {
...
Hooks.once("ready", () => {
  add2eInstallMonsterSheetPresentationPatch();
});
