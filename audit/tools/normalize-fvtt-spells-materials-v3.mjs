...
const VERSION = "2026-06-17-normalize-spell-materials-v3-druid-n7-materials-v1";
...
function cleanMaterial(value) {
  ...
  out = out.replace(/^typiquement\s+druidique\s*\((.*?)\)$/i, "$1").replace(/^à\s+savoir\s+/i, "").replace(/^d['’]\s*/i, "").replace(/^(un|une)?\s*peu\s+de\s+/i, "").replace(/^(un|une|du|de la|de l['’]?|des|le|la|les)\s+/i, "").replace(/^(quelques|plusieurs)\s+/i, "").replace(/^petit morceau de\s+/i, "").replace(/^morceau de\s+/i, "").replace(/[.!?;:]+$/g, "").trim();
...
function collectMaterials(value, out = [], notes = []) {
  ...
  const main = text(value).split(/\s*(?:optionnel|alternative|ingrédient matériel|ingredient materiel|composant matériel|composant materiel|composant requis|consomm|formulation source|manuel des joueurs|requise pour|requis pour|sort normal|sort inversé|description indique|pour lancer le sort)\b/i)[0];
...
function kindAndTags(name, classSlug) { const key = slug(name); const tags = [`effet:${key}`, `classe:${classSlug}`]; let kind = `${classSlug}_effect`; let automation = "active_effect_or_mj_aid"; if (/(soin|guerison|guérison|regeneration|régénération|restauration|rappel|resurrection|résurrection|reincarnation|réincarnation)/.test(key)) { kind = "healing_or_restoration"; tags.push("soin_ou_restauration"); } else if (/(detection|détection|localisation|prevision|prévision|communion|orientation|lithomancie|divination|augure)/.test(key)) { kind = "detection_or_divination"; automation = "mj_aid"; tags.push("detection_ou_divination"); } else if (/(langage|amitie|amitié|charme|perception)/.test(key)) { kind = "communication_or_control"; automation = "mj_aid"; tags.push("communication_ou_controle"); } else if (/(protection|bouclier|peau_d_ecorce|invisibilite|invisibilité|resistance|résistance)/.test(key)) { kind = "protection"; tags.push("protection"); } else if (/(paralysie|confusion|debilite|débilité|piege|piège|croc_en_jambe|repulsion|répulsion|quete|quête|exorcisme)/.test(key)) { kind = "control"; tags.push("controle"); } else if (/(feu|foudre|metal|métal|pyrotechnie|tempete|tempête|lames|epines|épines|mort_rampante|pilier|tremblement)/.test(key)) { kind = "damage_or_area_control"; automation = "damage_or_mj_aid"; tags.push("degats_ou_zone"); } else if (/(invocation|serviteur|animal|animaux|insectes|animation)/.test(key)) { kind = "summon_or_actor_effect"; automation = "actor_creation_or_mj_aid"; tags.push("invocation_ou_creation"); } else if (/(plante|plantes|vegetal|végétal|bois|arbre|shillelagh|croissance|batons|bâtons)/.test(key)) { kind = "plant_or_nature_control"; tags.push("plante_ou_nature"); } else if (/(eau|aquatique|climat|vents|vent|terre|pierre|roche|boue|separation|séparation|abaissement)/.test(key)) { kind = "elemental_or_weather_control"; automation = "mj_aid"; tags.push("element_ou_climat"); } return { kind, tags, automation }; }
...
function druidMaterialWarning(item) { const materials = item?.system?.composants_materiels ?? []; const hasM = String(item?.system?.composantes ?? "").toUpperCase().includes("M"); const bad = materials.filter(v => rejectMaterial(v) || norm(v).includes("druide doit") || norm(v).includes("cendres") || norm(v).includes("composant requis")); if (hasM && isDruideSpell(item) && !materials.length) bad.push("M sans composant"); return bad.length ? { name: item.name, niveau: item.system?.niveau, components: bad, allComponents: clone(materials) } : null; }
...
function main() {
  ...
  const json = JSON.parse(raw); const items = getItems(json); const control = { version: VERSION, input: path.relative(repoRoot, input), output: path.relative(repoRoot, output), totalItems: items.length, spells: 0, changedSpells: 0, changedEffectProfiles: 0, emptyMaterialSpells: 0, examples: [], watched: {}, suspiciousMaterialComponents: [], druideMaterialAudit: [], druideMaterialWarnings: [], sameSystemFieldsForAllSpells: true, canonicalFields: SYSTEM_KEYS };
  for (const level of CLERIC_LEVELS) control[`clercLevel${level}EffectProfiles`] = makeBucket(); for (const level of DRUID_LEVELS) control[`druideLevel${level}EffectProfiles`] = makeBucket(); const expected = JSON.stringify([...SYSTEM_KEYS].sort());
