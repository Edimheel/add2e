import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const repoRoot = path.resolve(__dirname, "../..");

const VERSION = "2026-06-17-normalize-spell-materials-v2";
const DEFAULT_INPUT = "fvtt-spells-all-normalise-mecanique-v1.json";
const DEFAULT_OUTPUT = "fvtt-spells-all-normalise-mecanique-v2.json";
const DEFAULT_CONTROL = "fvtt-spells-all-normalise-mecanique-v2-controle.json";

const TECHNICAL_SPLIT_RE = /\b(?:optionnel|alternative\s*:|ingr[eé]dient\s+mat[eé]riel|consomm[eé]|consomme|non[_\s-]*consomm[eé]|formulation\s+source|manuel\s+(?:du|des)\s+joueurs?|source\s*:|r[eé]f[eé]rence\s*:|r[eè]gle\s+d['’]arbitrage|selon\s+la\s+r[eè]gle).*$/i;
const TECHNICAL_VALUE_RE = /^(?:true|false|oui|non|consomme|non consomme|optionnel|manuel|manuel du joueur|manuel des joueurs|source|a completer|aucun|null|undefined)$/i;
const SYSTEM_KEYS = [
  "nom",
  "type",
  "classe",
  "spellLists",
  "niveau",
  "ecole",
  "portee",
  "duree",
  "zone_effet",
  "cible",
  "temps_incantation",
  "jet_sauvegarde",
  "composantes",
  "composants_materiels",
  "composants_materiels_source",
  "composants_materiels_reference",
  "composants_materiels_verification_recommandee",
  "composants_materiels_note",
  "composants_materiels_a_renseigner",
  "description",
  "onUse",
  "onUseCode",
  "tags",
  "effectTags",
  "effectProfile"
];

function clone(value) {
  return value === undefined ? undefined : JSON.parse(JSON.stringify(value));
}

function normalizeKey(value) {
  return String(value ?? "")
    .trim()
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[’']/g, "")
    .replace(/[^a-z0-9]+/g, "_")
    .replace(/_+/g, "_")
    .replace(/^_+|_+$/g, "");
}

function getItems(json) {
  if (Array.isArray(json)) return json;
  for (const key of ["items", "Item", "Items", "documents", "data", "entries"]) {
    if (Array.isArray(json?.[key])) return json[key];
  }
  return [];
}

function normalizeLabel(value) {
  return String(value ?? "")
    .replace(/[_-]+/g, " ")
    .replace(/\s+/g, " ")
    .replace(/^d['’]\s*/i, "")
    .replace(/^(un|une)?\s*peu\s+de\s+/i, "")
    .replace(/^(un|une|du|de la|de l['’]?|des|le|la|les)\s+/i, "")
    .replace(/^(quelques|plusieurs)\s+/i, "")
    .replace(/^petit morceau de\s+/i, "")
    .replace(/^morceau de\s+/i, "")
    .replace(/^symbole sacre$/i, "symbole sacré")
    .replace(/^gousse ail$/i, "gousse d’ail")
    .replace(/^poudre argent$/i, "poudre d’argent")
    .replace(/^eau benite$/i, "eau bénite")
    .replace(/^eau maudite$/i, "eau maudite")
    .replace(/[.!?;:]+$/g, "")
    .trim();
}

function isTechnicalValue(value) {
  const text = String(value ?? "").trim();
  if (!text) return true;
  if (/^-?\d+(?:[.,]\d+)?$/.test(text)) return true;
  const key = normalizeLabel(text)
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-z0-9 ]+/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  if (TECHNICAL_VALUE_RE.test(key)) return true;
  return key.startsWith("formulation source") || key.startsWith("alternative") || key.startsWith("ingredient materiel") || key.includes("manuel des joueurs");
}

function splitNameAndNote(value) {
  const raw = String(value ?? "").replace(/\s+/g, " ").trim();
  if (!raw) return { nameText: "", noteText: "" };

  const match = raw.match(TECHNICAL_SPLIT_RE);
  if (!match) return { nameText: raw, noteText: "" };

  const index = match.index ?? raw.length;
  return {
    nameText: raw.slice(0, index).replace(/[,;:\s]+$/g, "").trim(),
    noteText: raw.slice(index).replace(/^[,;:\s]+/g, "").trim()
  };
}

function addUnique(target, value) {
  const label = normalizeLabel(value);
  if (!label || isTechnicalValue(label)) return;
  const key = normalizeKey(label);
  if (!key || target.some(entry => normalizeKey(entry) === key)) return;
  target.push(label);
}

function addNote(notes, value) {
  const text = String(value ?? "").replace(/\s+/g, " ").trim();
  if (!text) return;
  if (notes.some(entry => normalizeKey(entry) === normalizeKey(text))) return;
  notes.push(text);
}

function collectMaterial(value, names, notes) {
  if (value === undefined || value === null || value === "") return;

  if (Array.isArray(value)) {
    for (const entry of value) collectMaterial(entry, names, notes);
    return;
  }

  if (typeof value === "object") {
    for (const key of ["note", "notes", "condition", "conditions", "description", "source", "reference", "formulation_source", "source_text"]) {
      if (value[key]) addNote(notes, value[key]);
    }

    const alternatives = value.alternatives ?? value.options ?? value.choix ?? value.auChoix ?? value.or;
    if (Array.isArray(alternatives) && alternatives.length) {
      const altNames = [];
      for (const alt of alternatives) collectMaterial(alt, altNames, notes);
      if (altNames.length) addUnique(names, altNames.join(" ou "));
      return;
    }

    const direct = value.nom ?? value.name ?? value.label ?? value.item ?? value.itemName ?? value.component ?? value.composant ?? value.slug ?? value.id;
    if (direct !== undefined && direct !== null && String(direct).trim()) {
      collectMaterial(String(direct), names, notes);
      return;
    }

    for (const [key, entry] of Object.entries(value)) {
      if (["quantite", "quantity", "qty", "nombre", "count", "consomme", "consume", "consumption", "consommation", "source", "reference", "note", "notes", "description", "condition", "conditions"].includes(key)) continue;
      collectMaterial(entry, names, notes);
    }
    return;
  }

  const { nameText, noteText } = splitNameAndNote(value);
  if (noteText) addNote(notes, noteText);

  for (const part of String(nameText).split(/[,;|\n]+/g).map(v => normalizeLabel(v)).filter(Boolean)) {
    addUnique(names, part);
  }
}

function normalizeMaterials(system) {
  const names = [];
  const notes = [];
  const before = clone(system.composants_materiels ?? []);

  collectMaterial(system.composants_materiels, names, notes);
  if (!names.length) collectMaterial(system.composants_materiels_objets, names, notes);
  if (!names.length) collectMaterial(system.composants_requis, names, notes);

  const oldNote = String(system.composants_materiels_note ?? "").trim();
  if (oldNote) addNote(notes, oldNote);

  system.composants_materiels = names;
  system.composants_materiels_note = notes.join("\n");

  return {
    changed: JSON.stringify(before) !== JSON.stringify(system.composants_materiels),
    before,
    after: clone(system.composants_materiels),
    notes: clone(notes)
  };
}

function ensureSystemShape(system) {
  for (const key of SYSTEM_KEYS) {
    if (!Object.prototype.hasOwnProperty.call(system, key)) {
      if (["spellLists", "composants_materiels", "tags", "effectTags"].includes(key)) system[key] = [];
      else if (key === "effectProfile") system[key] = { version: "2026-06-16-add2e-spell-effects-v1", source: "canonical-system", effects: [] };
      else if (key === "composants_materiels_a_renseigner") system[key] = false;
      else system[key] = "";
    }
  }
}

function main() {
  const input = path.resolve(repoRoot, process.argv[2] || DEFAULT_INPUT);
  const output = path.resolve(repoRoot, process.argv[3] || DEFAULT_OUTPUT);
  const controlOutput = path.resolve(repoRoot, process.argv[4] || DEFAULT_CONTROL);

  if (!fs.existsSync(input)) throw new Error(`Fichier introuvable : ${input}`);

  const json = JSON.parse(fs.readFileSync(input, "utf8"));
  const items = getItems(json);
  const control = {
    version: VERSION,
    input: path.relative(repoRoot, input),
    output: path.relative(repoRoot, output),
    totalItems: items.length,
    spells: 0,
    changedSpells: 0,
    emptyMaterialSpells: 0,
    examples: [],
    byClassLevel: {},
    sameSystemFieldsForAllSpells: true,
    canonicalFields: SYSTEM_KEYS
  };

  const expected = JSON.stringify([...SYSTEM_KEYS].sort());

  for (const item of items) {
    if (!item || String(item.type ?? item.system?.type ?? "") !== "sort") continue;
    item.system ??= {};
    ensureSystemShape(item.system);
    const result = normalizeMaterials(item.system);
    control.spells += 1;

    const classLevel = `${item.system.classe || "Classe inconnue"} / Niveau ${item.system.niveau || "inconnu"}`;
    control.byClassLevel[classLevel] = (control.byClassLevel[classLevel] ?? 0) + 1;

    if (!item.system.composants_materiels.length) control.emptyMaterialSpells += 1;
    if (result.changed) {
      control.changedSpells += 1;
      if (control.examples.length < 30) {
        control.examples.push({ name: item.name, before: result.before, after: result.after, notes: result.notes });
      }
    }

    const keys = JSON.stringify(Object.keys(item.system).sort());
    if (keys !== expected) control.sameSystemFieldsForAllSpells = false;
  }

  json.normalizedBy = VERSION;
  json.normalizedAt = new Date().toISOString();

  fs.writeFileSync(output, `${JSON.stringify(json, null, 2)}\n`, "utf8");
  fs.writeFileSync(controlOutput, `${JSON.stringify(control, null, 2)}\n`, "utf8");

  console.log(`[ADD2E][SPELL_MATERIALS_V2] ${control.spells} sort(s), ${control.changedSpells} modifié(s).`);
  console.log(`[ADD2E][SPELL_MATERIALS_V2] Output: ${path.relative(repoRoot, output)}`);
  console.log(`[ADD2E][SPELL_MATERIALS_V2] Control: ${path.relative(repoRoot, controlOutput)}`);
}

main();
